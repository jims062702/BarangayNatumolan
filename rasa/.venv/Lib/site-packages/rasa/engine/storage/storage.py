...
class ModelMetadata:
    ...
    def from_dict(cls, serialized: Dict[Text, Any]) -> ModelMetadata:
        ...
        Raises:
            ValueError: If required metadata keys are missing or invalid.
        """
        from rasa.engine.graph import GraphSchema

        missing_keys = [
            key for key in _MODEL_METADATA_REQUIRED_KEYS if key not in serialized
        ]
        if missing_keys:
            raise ValueError(
        ...
        try:
            ...
            raise ValueError(
                "Model metadata has an invalid 'trained_at' value."
            ) from error
        ...
        try:
            ...
            raise ValueError(
                "Model metadata has an invalid 'training_type' value: "
                f"{serialized['training_type']!r}"
            ) from error
        ...
        return ModelMetadata(
            ...
            core_target=serialized.get("core_target"),
            nlu_target=serialized["nlu_target"],
            language=serialized.get("language"),
