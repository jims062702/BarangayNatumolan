...
def _normalize_union_type(type_annotation: TypeAnnotation) -> TypeAnnotation:
    """Normalize PEP 604 ``X | Y`` unions for ``typing_utils`` (Python 3.13+)."""
    if isinstance(type_annotation, types.UnionType):
        return typing.Union[tuple(typing.get_args(type_annotation))]
...
def _validate_parent_return_type(
    ...
    if not typing_utils.issubtype(
        _normalize_union_type(parent_return_type),
        _normalize_union_type(required_type),
    ):
        parent_node_text = ""
        if parent_node:
...
def _validate_chitchat_dependencies(
    ...
    """Validate IntentlessPolicy when pattern_chitchat uses action_trigger_chitchat."""
...
def _validate_intentless_policy_responses(
    ...
    """Validate IntentlessPolicy has applicable responses when configured.

    Responses must come from the domain outside flows or from end-to-end stories.
    """
...
def get_component_index(schema: GraphSchema, component_class: Type) -> Optional[int]:
    """Extract the index of a component of the given class in the schema.

    Assumes node names end with the index, formatted as run_{ComponentName}{Index}.
    """
...
def validate_router_exclusivity(schema: GraphSchema) -> None:
    """Validate intent-based and llm-based routers are not both defined."""
...
def validate_intent_based_router_position(schema: GraphSchema) -> None:
    """Validate intent-based router is positioned before the llm command generator."""
...
def _validate_component_model_client_config_has_references_to_endpoints(
    ...
    """Validate client config references a model group in endpoints.yml.

    Ensures that when the client configuration for a component uses the
