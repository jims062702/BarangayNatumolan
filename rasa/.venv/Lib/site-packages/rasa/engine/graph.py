    ...
    def from_dict(cls, serialized_graph_schema: Dict[Text, Any]) -> GraphSchema:
        ...
        Raises:
            GraphSchemaException: In case the component class for a node couldn't be
                found, or the schema payload is malformed.
        """
        nodes_data = serialized_graph_schema.get("nodes")
        if not isinstance(nodes_data, dict):
            raise GraphSchemaException(
                "Graph schema is missing required 'nodes' mapping."
            )

        nodes = {}
        for node_name, serialized_node in nodes_data.items():
            try:
                ...
                resource = serialized_node.get("resource")
                if resource:
                ...
                raise GraphSchemaException(
                    ...
                    f"'{serialized_node.get('uses')}'."
                ) from e
            except KeyError as error:
                raise GraphSchemaException(
                    f"Graph schema node '{node_name}' is missing required field "
                    f"'{error.args[0]}'."
                ) from error
