...
from rasa.core.async_task_execution.async_task_result_processor import (
    AsyncTaskResultProcessor,
    InvalidAsyncTaskUpdateError,
)
...
class ErrorResponse(Exception):
    ...
    def __init__(
        ...
        self.status = status
        logger.error("error_response_exception", message=message)
        ...
        async def decorated(
            ...
            if token is not None and provided == token:
                ...
                raise ErrorResponse(
                    ...
                    help_url=DOCS_URL_HTTP_API_SECURITY,
                )
...
async def update_conversation_with_events(
    ...
    if tracker.terminated:
        logger.warning(
            "update_conversation_with_events.tracker.terminated",
            message=f"Attempting to add {len(events)} event(s) "
            f"to terminated conversation "
...
async def _load_agent(
    ...
    try:
        ...
        logger.debug(
            "_load_agent.internal_server_error", message=traceback.format_exc()
        )
        raise ErrorResponse(
    ...
    async def decorated_function(
        ...
        async def wrapped() -> None:
            try:
                ...
                logger.debug(
                    "async_if_callback_url.wrapped",
                    message="Asynchronous processing of request was successful. "
                ...
                if not isinstance(e, ErrorResponse):
                    logger.error(
                        "async_if_callback_url.wrapped.unexpected_error", error=e
                    )
                    e = ErrorResponse(
                        HTTPStatus.INTERNAL_SERVER_ERROR,
                        "UnexpectedError",
                ...
                logger.error(
                    "async_if_callback_url.wrapped.error_on_async_request_processing",
                    message="Error happened when processing request asynchronously. "
...
def register_a2a_server(
    app: Sanic,
    a2a_config: "A2AServerConfig",
    a2a_agent_card: Optional["AgentCard"] = None,
) -> None:
    """Register the A2A blueprint on a Sanic app."""
    from rasa.core.a2a_server.auth import A2ABearerAuthValidator
    from rasa.core.a2a_server.bootstrap import resolve_agent_card
    from rasa.core.a2a_server.sanic_app import agent_card_payload, create_a2a_blueprint

    initial_agent_card = a2a_agent_card or resolve_agent_card(a2a_config)
    app.ctx.a2a_agent_card_payload = agent_card_payload(initial_agent_card)
    app.ctx.a2a_bearer_auth_validator = (
        A2ABearerAuthValidator(a2a_config.auth) if a2a_config.auth else None
    )
    app.blueprint(create_a2a_blueprint())


def _call_with_event_loop(func: Callable[[], None]) -> None:
    """Call func, creating a temporary event loop if no running loop exists.

    Sanic uses fork-based multi-worker mode. Any event loop created here must be
    closed before app.run() forks workers — open kqueue/epoll fds are inherited by
    forked workers and cause random crashes (ENG-2606).
    """
    try:
...
async def _ensure_async_task_processor(app: Sanic) -> AsyncTaskResultProcessor:
    agent = app.ctx.agent
    current = getattr(app.ctx, "async_task_processor", None)

    if (
        current is not None
        and current.processor is agent.processor
        and current.tracker_store is agent.tracker_store
        and current.lock_store is agent.lock_store
    ):
        return current
    ...
    processor = AsyncTaskResultProcessor(
        lock_store=agent.lock_store,
        tracker_store=agent.tracker_store,
        processor=agent.processor,
        on_task_updated=lambda conversation_id, task_id: (
            _notify_input_channels_of_async_task_update(app, conversation_id, task_id)
        ),
    )
    app.ctx.async_task_processor = processor
    return processor


async def _notify_input_channels_of_async_task_update(
    app: Sanic, conversation_id: str, task_id: str
) -> None:
    agent = getattr(app.ctx, "agent", None)
    processor = getattr(agent, "processor", None)
    if processor is None:
        return

    input_channels = getattr(app.ctx, "input_channels", None) or []
    for input_channel in input_channels:
        try:
            delivered = await input_channel.on_async_task_update(
            ...
            logger.exception(
                "async_task_result.notify_input_channel.error",
        ...
        if delivered:
            logger.debug(
                "async_task_result.notify_input_channel.delivered",
                input_channel=input_channel.name(),
                conversation_id=conversation_id,
                task_id=task_id,
            )
...
async def _start_async_task_processor(app: Sanic, _: asyncio.AbstractEventLoop) -> None:
    if getattr(app.ctx, "agent", None) and app.ctx.agent.processor:
        await _ensure_async_task_processor(app)


async def _stop_async_task_processor(app: Sanic, _: asyncio.AbstractEventLoop) -> None:
    processor = getattr(app.ctx, "async_task_processor", None)
...
def create_app(
    ...
    if jwt_secret and jwt_method:
        ...
        # `sanic-jwt` needs an event loop when Initialize() is called.
        # _call_with_event_loop creates a temporary one if none exists and
        # closes it immediately after to prevent fd inheritance in fork workers.
        _call_with_event_loop(
            lambda: Initialize(
                app,
                secret=jwt_secret,
    ...
    app.ctx.agent = agent
    app.ctx.async_task_processor = None
    ...
    app.register_listener(_start_async_task_processor, "before_server_start")
    app.register_listener(_stop_async_task_processor, "after_server_stop")
    ...
    async def retrieve_tracker(request: Request, conversation_id: Text) -> HTTPResponse:
        ...
        try:
            ...
            logger.debug(
                "create_app.retrieve_tracker.conversation_error",
    ...
    async def delete_tracker(request: Request, conversation_id: Text) -> HTTPResponse:
        ...
        try:
            async with app.ctx.agent.lock_store.lock(conversation_id):
                ...
                logger.info(
                    "server.delete_tracker.complete",
                    message=f"Tracker for conversation '{conversation_id}' deleted.",
                )
            return response.empty(status=HTTPStatus.NO_CONTENT)
        except Exception as e:
            logger.debug("server.delete_tracker.error", message=traceback.format_exc())
            raise ErrorResponse(
                HTTPStatus.INTERNAL_SERVER_ERROR,
                "ConversationError",
                f"An unexpected error occurred. Error: {e}",
    ...
    async def get_trackers_by_user_id(request: Request, user_id: str) -> HTTPResponse:
        ...
        try:
            ...
            raise ErrorResponse(
                ...
                help_url=DOCS_URL_TRACKER_STORES,
            )
    ...
    async def _evaluate_model_using_test_set(
        ...
        try:
            ...
            return await rasa.nlu.test.run_evaluation(
                data_path,
                eval_agent.processor,
                disable_plotting=True,
                report_as_dict=True,
            )
        finally:
            # If a temporary agent was loaded for this evaluation, stop its privacy
            # manager background thread. The guard avoids stopping the live agent
            # when no separate model was loaded (``eval_agent is app.ctx.agent``).
            if eval_agent is not app.ctx.agent:
                eval_agent.stop_privacy_manager()
    ...
    async def load_model(request: Request) -> HTTPResponse:
        ...
        # Capture the retired agent immediately before the swap (no await in
        # between) so its privacy manager background thread can be stopped.
        old_agent = app.ctx.agent
        app.ctx.agent = new_agent
        if old_agent is not new_agent:
            old_agent.stop_privacy_manager()
    ...
    async def unload_model(request: Request) -> HTTPResponse:
        old_agent = app.ctx.agent
        model_file = old_agent.model_name

        app.ctx.agent = Agent(lock_store=old_agent.lock_store)
        old_agent.stop_privacy_manager()
    ...
    @app.post("/agent/task_update")
    @requires_auth(app, auth_token)
    @ensure_loaded_agent(app)
    async def update_agent_task(request: Request) -> HTTPResponse:
        """Receive and queue an update on the agent's task."""
        validate_request_body(
            request,
            "No task update defined in request body. Add a task payload to the "
            "request body in order to update the conversation state.",
        )

        logger.debug(
            "http.post.agent.task_update",
            message="Received agent task update",
            request_body=request.json,
        )

        processor = await _ensure_async_task_processor(request.app)

        try:
            await processor.process_push_notification(request.json)
        except InvalidAsyncTaskUpdateError as e:
            raise ErrorResponse(
                HTTPStatus.BAD_REQUEST,
                "BadRequest",
                str(e),
