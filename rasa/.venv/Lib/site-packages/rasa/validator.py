...
class Validator:
    ...
    def verify_mcp_pre_call_hooks(self) -> bool:
        """Validate MCP server ``pre_call_hook`` import paths in endpoints.yml.

        Returns:
            False if any configured hook path cannot be resolved to a callable.
        """
        from rasa.exceptions import ValidationError
        ...
        for server in endpoints.mcp_servers:
            if not server.pre_call_hook:
                continue
            if server.pre_call_hook in seen_hook_paths:
                continue
            seen_hook_paths.add(server.pre_call_hook)
            try:
                validate_pre_call_hook_import(
                    server.pre_call_hook,
                    context_label=f"MCP server '{server.name}'",
                )
            except ValidationError as e:
                structlogger.error(
                    "validator.verify_mcp_pre_call_hooks",
                    event_info=str(e),
                    mcp_server=server.name,
                    pre_call_hook=server.pre_call_hook,
                )
                return False
        return True
