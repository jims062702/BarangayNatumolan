...
def _instrument_execute_mcp_tool_call(
    ...
    async def traced_execute_mcp_tool_call(
        ...
        with tracer.start_as_current_span(
            ...
            start_time = time.perf_counter_ns()
            # Capture before execution: initial_events is mutated in place and
            # result.events is the same list object.
            ...
            # Count events appended during tool execution (list grows on success
            # and error).
            tool_generated_events_count = len(result.events) - initial_events_count
            ...
            tool_output_events = []
            if result.events and len(result.events) > initial_events_count:
                for event in result.events[initial_events_count:]:
