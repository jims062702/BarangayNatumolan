...
# LiteLLM env var (not a Langfuse one): as of litellm 1.87.0 the LLM request
# generation span is only created when this is truthy or no parent OTEL span is
# active. Rasa's command generator always runs inside an ``@observe`` parent
# span, so we force this on when Langfuse is configured (see ENG-2929).
LITELLM_ENV_VAR_USE_OTEL_REQUEST_SPAN = "USE_OTEL_LITELLM_REQUEST_SPAN"
