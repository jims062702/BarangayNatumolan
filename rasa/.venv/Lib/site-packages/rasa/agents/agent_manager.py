...
class AgentManager(metaclass=Singleton):
    ...
    async def run_async_agent(
        ...
        Args:
            protocol_type: The protocol type of the agent.
            agent_input: The input to the agent as an AgentInput object.
            output_channel: The output channel to use.
            cancellation_token: Optional token for cooperative cancellation.
        ...
        structlogger.debug(
            "agent_manager.run_async_agent.input",
            event_info="Processing agent input before sending...",
            agent_name=agent_name,
            protocol_type=protocol_type,
        )
        ...
        # Process input before sending
        try:
            processed_input = await agent.process_input(agent_input)
        except Exception as e:
            structlogger.error(
                "agent_manager.run_async_agent.process_input_failed",
                agent_name=agent_name,
                protocol_type=protocol_type,
                event_info=(
                    f"Failed to process input for agent '{agent_name}'. "
        ...
        if cancellation_token is None:
            output = await agent.run_async(
                processed_input, output_channel=output_channel
            )
        else:
            output = await agent.run_async(
                processed_input,
        ...
        structlogger.debug(
            "agent_manager.run_async_agent.output",
            event_info="Agent output received. Processing output...",
            agent_name=agent_name,
            protocol_type=protocol_type,
        )
        ...
        if isinstance(agent, A2AAgent):
            try:
                ...
                structlogger.error(
                    "agent_manager.run_async_agent.process_agent_output_failed",
                    agent_name=agent_name,
                    protocol_type=protocol_type,
                    event_info=(
                        f"Failed to process output for agent '{agent_name}'. "
