...
from rasa.shared.utils.langfuse import langfuse_session_id_override
...
class MCPBaseAgent(AgentProtocol):
    ...
    def __init__(
        ...
        # Server name -> call-time credential hook import path from endpoints.yml.
        self._server_to_pre_call_hook: Dict[str, str] = {
            server_config.name: server_config.pre_call_hook
            for server_config in self._server_configs
            if server_config.pre_call_hook is not None
        }
    ...
    def create_bot_uttered_for_streamed_content(
        ...
        llm_time_to_first_token_ms: Optional[float] = None,
        ...
        in the tracker and downstream analytics. LLM timing metrics are included
        when available.
        ...
        Args:
            ...
            llm_time_to_first_token_ms: Time to first non-empty content delta,
                in milliseconds. None for non-streaming responses.
            llm_total_time_ms: Total LLM call duration, in milliseconds.
        ...
        return BotUttered(
            ...
            metadata={
                ...
                LLM_TIME_TO_FIRST_TOKEN_MS_KEY: llm_time_to_first_token_ms,
                LLM_TOTAL_TIME_MS_KEY: llm_total_time_ms,
            },
        )
    ...
    def get_llm_tracing_metadata(self, agent_input: AgentInput) -> Dict[str, Any]:
        sender_id = agent_input.metadata.get(AGENT_METADATA_SENDER_ID_KEY)
        override = langfuse_session_id_override.get()
        return {
            LANGFUSE_METADATA_SESSION_ID: override
    ...
    async def generate_and_send_response(
        ...
        if llm_content and llm_response is not None:
            llm_total_time_ms = (
                llm_response.latency * 1000
                if llm_response.latency is not None
                else None
            )
            bot_uttered = self.create_bot_uttered_for_streamed_content(
                llm_content,
                agent_input,
                llm_time_to_first_token_ms=llm_response.llm_time_to_first_token_ms,
    ...
    async def build_custom_tool_call_metadata(
        self, context: MCPOutboundCallContext
    ) -> OutboundCallResult:
        """Return call-time credential metadata for a ReAct custom-tool call.

        Override this on a subclass to fetch and decrypt credentials from an
        external secret store (keyed by ``context.sender_id``) and return an
        ``OutboundCallResult``. Its ``metadata`` is merged into the custom
        tool's ``AgentToolContext.metadata``; it is never written to slots, the
        tracker, Kafka, or the inspector.

        The default implementation is a no-op (returns an empty
        ``OutboundCallResult``), preserving existing behaviour.

        External MCP tools do not use this method: they resolve their hook from
        the MCP server's ``pre_call_hook`` in ``endpoints.yml``.
        """
        return OutboundCallResult()

    @staticmethod
    def build_mcp_outbound_call_context_for_agent(
        agent_input: AgentInput,
        tool_name: str,
        server_id: Optional[str] = None,
    ) -> MCPOutboundCallContext:
        """Build ``MCPOutboundCallContext`` for a ReAct MCP or custom-tool call."""
        metadata = agent_input.metadata or {}
        sender_id = (
            agent_input.recipient_id or metadata.get(AGENT_METADATA_SENDER_ID_KEY) or ""
        )
        return MCPOutboundCallContext(
            sender_id=sender_id,
    ...
    async def _resolve_mcp_call_metadata(
        self,
        agent_input: Optional[AgentInput],
        server_id: str,
        tool_name: str,
    ) -> Optional[OutboundCallResult]:
        """Invoke the MCP server's ``pre_call_hook`` (from endpoints), if any.

        Returns ``None`` when no hook is configured or no ``agent_input`` is
        available (the context cannot be built). Raises ``OutboundCallHookError``
        on hook failure so the caller can fail-fast without calling the tool.
        """
        if agent_input is None:
            return None
        import_path = self._server_to_pre_call_hook.get(server_id)
        if not import_path:
            return None
        hook = load_call_hook(import_path)
        context = self.build_mcp_outbound_call_context_for_agent(
            agent_input=agent_input,
            server_id=server_id,
            tool_name=tool_name,
        )
        return await invoke_mcp_call_hook(hook, context)

    async def _resolve_custom_tool_call_metadata(
        self,
        agent_input: Optional[AgentInput],
        tool_name: str,
    ) -> Optional[OutboundCallResult]:
        """Invoke ``build_custom_tool_call_metadata`` for a custom-tool call.

        Returns ``None`` when no ``agent_input`` is available. Raises
        ``OutboundCallHookError`` on hook failure so the caller can fail-fast.
        """
        if agent_input is None:
            return None
        context = self.build_mcp_outbound_call_context_for_agent(
        ...
        return await invoke_mcp_call_hook(self.build_custom_tool_call_metadata, context)
    ...
    async def _execute_mcp_tool(
        ...
        try:
            hook_result = await self._resolve_mcp_call_metadata(
                agent_input=agent_input,
                server_id=server_id,
                tool_name=tool_name,
            )
            meta = merge_call_metadata(meta, hook_result)
            ...
            return AgentToolResult(
                ...
                is_error=True,
                error_message=(
                    f"Call metadata hook failed for tool `{tool_name}` on MCP "
                    f"server `{server_id}`: {e!s}"
                ),
            )
    ...
    async def _run_custom_tool(
        ...
        # Merge call-time credential hook output into the tool
        # context metadata. Fail-fast: on hook failure return an error result.
        try:
            hook_result = await self._resolve_custom_tool_call_metadata(
                agent_input=agent_input,
            ...
            return AgentToolResult(
                ...
                is_error=True,
                error_message=(
                    f"Call metadata hook failed for custom tool "
                    f"`{custom_tool.tool_name}`: {e!s}"
                ),
            )

        context = AgentToolContext(
            metadata=merge_call_metadata(base_metadata, hook_result)
        )
        result = custom_tool.tool_executor(executor_args, context)
        return await result if isawaitable(result) else result

    @observe(
        name="mcp_agent.execute_tool_call",
        as_type="generation",
        capture_input=False,
    ...
    async def _execute_tool_call(
        ...
        update_current_span_metadata(
            metadata={
                "prompt_type": "agent_tool_call",
                "agent_name": self._name,
                "tool_name": tool_name,
                "has_agent_input": agent_input is not None,
            }
        )
