...
from rasa.agents.constants import A2A_TASK_POLLING_MAX_WAIT
...
from rasa.core.lock_store import get_lock_lifetime
from rasa.exceptions import ValidationError
from rasa.shared.agents.auth.utils import validate_secrets_in_params
from rasa.shared.agents.outbound_call_hook import validate_pre_call_hook_import
...
def _validate_a2a_config(agent_config: AgentConfig) -> None:
    ...
    _validate_a2a_lock_lifetime(agent_config)


def _validate_a2a_lock_lifetime(agent_config: AgentConfig) -> None:
    """Validate that TICKET_LOCK_LIFETIME is at least max_polling_time."""
    agent_name = agent_config.agent.name
    max_polling_time = (
        agent_config.configuration.max_polling_time
        if agent_config.configuration and agent_config.configuration.max_polling_time
        else A2A_TASK_POLLING_MAX_WAIT
    )
    lock_lifetime = get_lock_lifetime()

    if lock_lifetime < max_polling_time:
        raise ValidationError(
            code="agent.validation.a2a.lock_lifetime_too_short",
            event_info=(
                f"For protocol 'A2A', agent '{agent_name}' has "
                f"max_polling_time={max_polling_time}s but TICKET_LOCK_LIFETIME is "
                f"{lock_lifetime}s. Set TICKET_LOCK_LIFETIME >= max_polling_time "
                "to prevent concurrent tracker writes."
            ),
        )
...
def _validate_optional_keys(agent_config: AgentConfig) -> None:
    ...
    # Validate pre_call_hook if present (A2A outbound credential hook only)
    if agent_config.configuration and agent_config.configuration.pre_call_hook:
        if agent_config.agent.protocol != ProtocolConfig.A2A:
            raise ValidationError(
                code="agent.validation.pre_call_hook.a2a_only",
                event_info=(
                    f"Agent '{agent_name}' has 'configuration.pre_call_hook' but "
                    f"protocol is '{agent_config.agent.protocol.value}'. "
                    f"pre_call_hook is only supported for A2A agents; configure MCP "
                    f"hooks on mcp_servers[].pre_call_hook in endpoints.yml instead."
                ),
            )
        validate_pre_call_hook_import(
            agent_config.configuration.pre_call_hook,
            context_label=f"agent '{agent_name}'",
        )
