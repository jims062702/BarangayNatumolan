...
class TrainingSession(BaseModel):
    ...
    deploy: bool = False
    deployed: bool = False
    zip_bot_configs: Optional[Dict[str, bytes]] = None
    base_url: str = ""
...
def seed_training_directory_with_rasa_cache(
    ...
    if os.path.exists(cache_path):
        ...
        dst = subpath(training_base_path, RASA_DIR_NAME)
        shutil.rmtree(dst, ignore_errors=True)
        shutil.copytree(src=cache_path, dst=dst)
...
def _read_subdirectory_files(
    base_path: str, subdirectory: str, configs: Dict[str, bytes]
) -> None:
    """Read all files under a subdirectory into configs keyed by relative path."""
...
def read_bot_configs_from_dir(base_path: str) -> Dict[str, bytes]:
    """Read bot config files from a directory as raw bytes.

    Returns a dict keyed by short name for the three main config files
    (``"config"``, ``"credentials"``, ``"endpoints"``) and by relative path
    for any files found under the ``prompts/`` and ``sub_agents/`` subdirectories.
    """
    ...
    _read_subdirectory_files(base_path, "prompts", configs)
    _read_subdirectory_files(base_path, "sub_agents", configs)
...
def start_training_process(
    ...
    model_name = model_name or generate_random_model_name()
    domain_arg = (
        "domain"
    ...
    return TrainingSession(
        ...
        deploy=deploy,
        zip_bot_configs=zip_bot_configs,
        base_url=base_url,
    )
...
def run_training(
    ...
    if zip_bytes is not None:
        ...
        if deploy:
            zip_bot_configs = read_bot_configs_from_dir(training_base_path)
    else:
