...
def split_e2e_test_suite(
    ...
    train_keys = {(tc.name, tc.file) for tc in train_test_cases}
    validation_keys = {(tc.name, tc.file) for tc in validation_test_cases}
    train_fixtures_per_test = [
        tcf
        for tcf in e2e_test_suite.fixtures_per_test
        if (tcf.test_case_name, tcf.file) in train_keys
    ]
    validation_fixtures_per_test = [
        tcf
        for tcf in e2e_test_suite.fixtures_per_test
        if (tcf.test_case_name, tcf.file) in validation_keys
    ]

    # Creating new TestSuite instances for train and validation
    train_suite = TestSuite(
        test_cases=train_test_cases,
        fixtures_per_test=train_fixtures_per_test,
    ...
    validation_suite = TestSuite(
        ...
        fixtures_per_test=validation_fixtures_per_test,
        metadata=e2e_test_suite.metadata,
        stub_custom_actions=e2e_test_suite.stub_custom_actions,
    )
...
def split_llm_fine_tuning_data(
    ...
    # Write the train and validation test suite data as per-source-file YAMLs
    # so per-file fixture scoping round-trips correctly (ENG-2647).
    storage_context.write_e2e_test_suite_to_directory(
        train_suite,
        TRAIN_TEST_MODULE_STORAGE_LOCATION,
        "e2e_tests/train",
        input_folder,
