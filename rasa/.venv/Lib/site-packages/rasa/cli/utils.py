...
def get_failed_e2e_tests_directory(
    failed_tests_path: Path,
) -> str:
    """Returns a timestamped directory for the e2e failed tests output.

    The directory mirrors the original test folder structure (ENG-2647) so a
    multi-source suite can be re-read without collapsing per-file fixture
    scoping. The timestamp keeps consecutive runs from overwriting each
    other when the user supplies the same root path.

    Args:
        failed_tests_path: Directory provided by the user via CLI for failed
            tests output. If the user supplied a `.yml`/`.yaml` file path
            (legacy form), its parent directory is used and the stem is
            kept as the subdirectory name for continuity.

    Returns:
        Path to the failed tests directory with timestamp as a string.
    """
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

    if failed_tests_path.is_dir() or not failed_tests_path.suffix:
        directory = failed_tests_path / f"e2e_failed_tests_{timestamp}"
    else:
        parent = failed_tests_path.parent
        stem = failed_tests_path.stem
        directory = parent / f"{stem}_{timestamp}"
