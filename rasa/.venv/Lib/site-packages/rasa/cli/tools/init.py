...
def run_wizard(args: argparse.Namespace) -> None:
    ...
    config = (
        ...
        if non_interactive
        else _run_interactive(project_dir, args)
    )
    ...
    if config.ide_integrations:
        if non_interactive:
            install_skills = getattr(args, "skills", False)
        elif getattr(args, "skills", False):
            install_skills = True
        else:
            install_skills = _ask_install_agent_skills()
        if install_skills:
            install_agent_skills(
                project_dir,
...
def _run_interactive(
    project_dir: Path, args: Optional[argparse.Namespace] = None
) -> RunConfig:
    ...
    IDE selections using the terminal UI.  Values already provided via CLI
    flags are used as-is and the corresponding prompts are skipped.

    Args:
        project_dir: Absolute path to the Rasa project root.
        args: Parsed CLI arguments; omitted flags are collected interactively.
    ...
    mode = getattr(args, "mode", None) or _ask_mode()
    ...
    if mode == MCP_TOOLS_TRANSPORT_HTTP:
        ...
        config_kwargs["port"] = port if port is not None else _ask_port()

    rasa_server_url = getattr(args, "rasa_server_url", None)
    config_kwargs["rasa_server_url"] = (
        rasa_server_url if rasa_server_url is not None else _ask_rasa_server_url()
    )

    docs_mode = getattr(args, "docs", None) or _ask_docs_mode()
    if raw_ides := getattr(args, "ides", None):
        ides = [tok.strip().lower() for tok in raw_ides.split(",") if tok.strip()]
    else:
        ides = _ask_ides()
...
def _write_codex_config(project_dir: Path, config: RunConfig) -> None:
    """Configure OpenAI Codex to use the Rasa MCP server.

    Always writes project-local ``.codex/config.toml`` with ``cwd``, absolute
    paths, and timeouts — required for the Codex desktop app.  Best-effort
    also registers the server globally via ``codex mcp add`` for CLI users.
    ...
    config_path = resolved_project / ".codex" / "config.toml"
    _merge_codex_toml(config_path, resolved_project, config)
    console.print(f"  [green]✔[/green] OpenAI Codex   → {config_path}")

    codex_bin = shutil.which("codex")
    if codex_bin and _codex_mcp_add(codex_bin, resolved_project, config):
        console.print(
            "  [dim]Also registered globally for Codex CLI "
            "([cyan]codex mcp list[/cyan])[/dim]"
        )


def _merge_codex_toml(path: Path, project_dir: Path, config: RunConfig) -> None:
    """Write or update ``[mcp_servers.rasa-tools]`` in a Codex config file.

    Other MCP servers and non-MCP settings in the file are preserved.

    Args:
        path: Target ``config.toml`` path (typically ``.codex/config.toml``).
        project_dir: Resolved absolute path to the Rasa project root.
        config: The resolved run configuration.
    """
    import tomlkit
    ...
    if config.mode == MCP_TOOLS_TRANSPORT_STDIO:
        entry = _build_stdio_entry(project_dir, config)
        server_table.add("command", entry["command"])
        server_table.add("args", entry["args"])
        server_table.add("cwd", str(project_dir))
    else:
        server_table.add("url", _build_http_entry(config.port)["url"])

    server_table.add("tool_timeout_sec", CODEX_TOOL_TIMEOUT_SEC)
    server_table.add("startup_timeout_sec", CODEX_STARTUP_TIMEOUT_SEC)
    server_table.add("enabled", True)

    doc["mcp_servers"]["rasa-tools"] = server_table

    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(tomlkit.dumps(doc))


def _codex_mcp_add(codex_bin: str, project_dir: Path, config: RunConfig) -> bool:
    """Try to register the server via ``codex mcp add``.
...
_IDE_NEXT_STEPS = {
    ...
    "codex": (
        "[bold]Codex:[/bold] Open this project folder in the Codex app and "
        "trust it when prompted. Go to Settings \u2192 Integrations & MCP "
        "and confirm [cyan]rasa-tools[/cyan] is enabled. Start a new thread "
        "if tools do not appear; restart Codex if needed. CLI users can "
        "verify with [cyan]codex mcp list[/cyan]."
    ),
}
