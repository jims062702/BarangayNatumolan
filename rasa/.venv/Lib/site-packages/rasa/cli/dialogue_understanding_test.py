...
def execute_dialogue_understanding_tests(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        # Ensure processor is not None so that we can extract the flows and the
        # llm config
        if test_runner.agent.processor is None:
            rasa.shared.utils.cli.print_error(
                "No processor: Not able to retrieve flows and config from "
                "trained model."
            )
            sys.exit(0)
        ...
        test_suite: TestSuite = get_valid_test_suite(
            args, flows, test_runner.agent.domain
        )
