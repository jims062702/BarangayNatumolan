...
def add_e2e_test_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Directory under which failed end-to-end tests are written. The "
            "input folder structure is mirrored inside a timestamped "
            "subdirectory so fixture scoping is preserved on reload. "
...
def execute_e2e_tests(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if failed and args.e2e_failed_tests is not None:
            ...
            failed_tests_dir = rasa.cli.utils.get_failed_e2e_tests_directory(
                failed_tests_path
            )
            write_failed_tests_to_file(
                test_suite.test_cases,
        ...
        # The privacy manager (if configured) runs a background scheduler thread
        # that must be stopped, otherwise the process never exits cleanly.
        test_runner.agent.stop_privacy_manager()
