...
class BackgroundPrivacyManager:
    ...
    def stop(self) -> None:
        ...
        # Idempotent: callers may stop a manager more than once (e.g. agent swap
        # on model reload followed by server shutdown). ``shutdown`` would raise
        # ``SchedulerNotRunningError`` on an already-stopped scheduler.
    ...
    def _get_processed_events_after_anonymization_env_set(
        ...
        """Env-set path: time-based anonymization.
    ...
    def _get_processed_events_after_anonymization_legacy(
        ...
        """Legacy path: no session_id.

        Split by action_session_start or inactive/ended when only one; time-based
        with LEGACY_DEFAULT_INACTIVITY. Returns (all_events in order, count of
        events anonymized this run).
        """
    ...
    def _get_processed_events_after_anonymization_event_only(
        ...
        """Event-only path (new/hybrid trackers).

        Process by runs to preserve chronological order; anonymize segments with
        ConversationInactive or SessionEnded only after min_after_session_end has
        elapsed.
    ...
    def _get_events_to_be_retained_after_deletion_env_set(
        ...
        """Env-set path: time-based retention.

        Session splitting respects new/hybrid (group by session_id; legacy segment
        split). Uses events only.
        """
        sessions = self._get_env_set_session_trackers(full_tracker)
