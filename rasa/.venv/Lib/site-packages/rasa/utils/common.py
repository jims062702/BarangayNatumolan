...
EXPECTED_WARNINGS: List[Tuple[Type[Warning], str]] = [
    ...
    # jsonpickle 5.0.0: keys will default to True (benign behavior change warning)
    (DeprecationWarning, "keys will default to True in jsonpickle.*"),
]
...
def configure_library_logging() -> None:
    """Configures log levels of used libraries such as kafka, matplotlib, pika, a2a."""
    ...
    update_a2a_log_level(library_log_level)
...
def update_a2a_log_level(library_log_level: Text) -> None:
    """Set the log level for a2a-sdk loggers.

    The ``a2a.*`` loggers emit verbose ``DEBUG`` lines during event-queue
    polling, telemetry, and task lifecycle handling that are typically noise
    when Rasa runs as an A2A server.  Verbosity is controlled via the shared
    ``LOG_LEVEL_LIBRARIES`` environment variable.
    """
    log_level = os.environ.get(ENV_LOG_LEVEL_LIBRARIES, library_log_level)
