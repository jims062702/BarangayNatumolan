...
from rasa.builder.document_retrieval.inkeep.constants import INKEEP_BASE_URL_ENV_VAR
...
MESSAGE_CLASSIFIER_CHAT_HISTORY_SIZE = int(
    os.getenv("MESSAGE_CLASSIFIER_CHAT_HISTORY_SIZE", "5")
)

# Document Retrieval Configuration
# The LOCAL_* settings are read only for the local retrieval to load and index docs
LOCAL_DOCS_INDEX_PATH = os.getenv("LOCAL_DOCS_INDEX_PATH", ".retrieval_docs_index")
LOCAL_INDEX_CACHE_DIR = os.getenv("LOCAL_INDEX_CACHE_DIR", ".retrieval_cache_index")
# Baked embedding-model dir; None => download on first use.
LOCAL_EMBED_CACHE_DIR = os.getenv("LOCAL_EMBED_CACHE_DIR", ".embed_model_cache")
# Prebuilt parquet vector artifact. When set, the copilot ingests precomputed
# vectors (pull-and-serve, no embedding) instead of building from the corpus.
LOCAL_VECTORS_PARQUET_PATH = os.getenv("LOCAL_VECTORS_PARQUET_PATH")

# Copilot Agent Configuration
COPILOT_MAX_AGENT_STEPS = int(os.getenv("COPILOT_MAX_AGENT_STEPS", "50"))
COPILOT_V2_ENABLED = os.getenv("COPILOT_V2_ENABLED", "false").lower() == "true"
# Copilot v1 API path — used as the v2 kill-switch fallback and telemetry tag.
COPILOT_V1_API_PATH = "/api/copilot"
