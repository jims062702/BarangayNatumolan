...
from rasa.builder.document_retrieval.local.config import LocalRetrievalConfig
...
class RetrievalTaskConfig(BaseModel):
    ...
    backend: Literal["inkeep", "local"] = Field(
        default="inkeep",
        description="Retrieval backend to evaluate.",
    )
    docs_index_path: Optional[str] = Field(
        default=None,
        description="Path to the prebuilt docs_index.jsonl corpus to embed and "
        "evaluate against (required when backend='local').",
    )
    local: Optional[LocalRetrievalConfig] = Field(
        default=None,
        description="Configuration for the local backend (required when "
        "backend='local').",
    )

    @model_validator(mode="after")
    def _require_local_config_for_local_backend(self) -> "RetrievalTaskConfig":
        """Fail fast if the local backend is selected without its config."""
        if self.backend == "local" and (
            self.local is None or self.docs_index_path is None
        ):
            raise ValueError(
