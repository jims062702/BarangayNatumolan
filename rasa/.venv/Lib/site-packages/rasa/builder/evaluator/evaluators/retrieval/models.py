...
class TokenStats(BaseModel):
    """Retrieved-content token statistics across queries (excluding errors).

    Measures the downstream LLM context cost of retrieval: tokens summed over
    all returned document contents per query, tokenized with the consumer model.
    """
...
class RetrievalMetricsSummary(BaseModel):
    ...
    tokens: TokenStats = Field(
        description="Retrieved-content token usage across non-error queries."
    )
