...
def mrr_at_k(retrieved: List[str], relevant: Set[str], k: int) -> float:
    """Reciprocal rank of the first relevant URL within the top-k delivered.

    Mirrors ``recall_at_k``: takes the top-k retrieved items (what is actually
    delivered downstream), then deduplicates to pages, so two chunks from the
    same URL count as one. Returns the reciprocal of the first relevant page's
    rank, or 0.0 if no relevant URL is in the top-k (or ``relevant`` is empty).
    """
    if not relevant:
        return 0.0

    top_k = deduplicate_urls(retrieved[:k])
    for rank, url in enumerate(top_k, start=1):
        if url in relevant:
            return 1.0 / rank
    return 0.0
