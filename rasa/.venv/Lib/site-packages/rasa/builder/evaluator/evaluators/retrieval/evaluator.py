...
class RetrievalEvaluator(BaseEvaluator):
        ...
        for item_result in item_results:
            ...
            results.append(
                RetrievalResult(
                    ...
                    retrieved_tokens=output.retrieved_tokens,
    ...
    def evaluate(self, results: List[RetrievalResult]) -> RetrievalMetricsSummary:
        ...
        for k in RECALL_KS:
            overall_recall[k] = mean(
                [
                    recall_at_k(r.retrieved_urls, set(r.relevant_urls), k)
                    for r in scorable
                ]
            )
            overall_mrr[k] = mean(
                [mrr_at_k(r.retrieved_urls, set(r.relevant_urls), k) for r in scorable]
            )
        ...
        # Retrieved-content token usage over non-error queries (downstream
        # context cost). Measured over all answered queries, regardless of
        # whether they have ground truth.
        token_values = [r.retrieved_tokens for r in non_error_results]
        token_stats = TokenStats(
            mean=mean(token_values),
            p50=percentile(token_values, 50.0),
            p95=percentile(token_values, 95.0),
        ...
        for category, category_results in by_category.items():
            per_category[category] = PerCategoryMetrics(
                ...
                mrr_at_3=mean(
                    [
                        mrr_at_k(r.retrieved_urls, set(r.relevant_urls), 3)
                        for r in category_results
                    ]
                ),
                mrr_at_5=mean(
                    [
                        mrr_at_k(r.retrieved_urls, set(r.relevant_urls), 5)
                        for r in category_results
                    ]
                ),
                mrr_at_10=mean(
                    [
                        mrr_at_k(r.retrieved_urls, set(r.relevant_urls), 10)
                        for r in category_results
                    ]
                ),
                # Token mean is over the same (scorable) queries used for recall
                # in this category, so the per-category view is self-consistent.
                tokens_mean=mean([r.retrieved_tokens for r in category_results]),
        ...
        return RetrievalMetricsSummary(
            ...
            mrr_at_3=overall_mrr[3],
            mrr_at_5=overall_mrr[5],
            mrr_at_10=overall_mrr[10],
    ...
    def to_evaluations(
        ...
        for name, value in [
            ...
            ("tokens_mean", summary.tokens.mean),
            ("tokens_p50", summary.tokens.p50),
            ("tokens_p95", summary.tokens.p95),
            ("tokens_max", float(summary.tokens.max)),
        ]:
            evaluations.append(
                Evaluation(
                    name=name,
        ...
        for category, metrics in summary.per_category.items():
            ...
            for metric_name, metric_value in [
                ...
                ("tokens_mean", metrics.tokens_mean),
