...
def create_app(project_folder: str) -> Sanic:
    ...
    # Warm the document-retrieval backend once at startup so it lives for the
    # whole service session. Only relevant for the agent_sdk copilot,
    # whose doc-search tool reads this process-wide singleton.
    @app.after_server_start
    async def init_retrieval_on_startup(
        app: Sanic, loop: asyncio.AbstractEventLoop
    ) -> None:
        """Build the document retriever once when the server starts."""
        if not config.USE_AGENT_SDK_COPILOT:
            return
        try:
            ...
            get_retriever()
            structlogger.info("Document retriever initialized on server startup")
        except Exception as e:
            structlogger.warning(
                "Failed to initialize document retriever on server startup",
                error=str(e),
            )
