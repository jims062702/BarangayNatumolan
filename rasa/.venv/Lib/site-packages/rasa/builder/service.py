...
@bp.route("/copilot/v2", methods=["POST"])
@openapi.summary("Copilot v2 (PydanticAI + AG-UI SSE)")
@openapi.description(
    "Production Copilot v2 endpoint. Streams AG-UI JSON events as "
    "Server-Sent Events using `data: {...}` lines. Disabled by default; "
    "set COPILOT_V2_ENABLED=true to enable."
)
@openapi.tag("copilot")
@openapi.body(
    {"application/json": {str: Any}},
    description=(
        "AG-UI RunAgentInput-shaped JSON with `threadId`, `runId`, and "
        "`messages` (list of objects with `role` and `content`). Optional "
        "`project_path`."
    ),
    required=True,
)
@openapi.response(
    200,
    {"text/event-stream": {str: Any}},
    description="SSE stream of AG-UI-shaped JSON objects, one per `data:` frame.",
)
@openapi.response(
    503,
    {"application/json": {str: Any}},
    description="Copilot v2 disabled (kill-switch). Frontend should fall back to v1.",
)
@protected()
async def copilot_v2(request: Request) -> Union[None, HTTPResponse]:
    """Stream AG-UI events for Copilot v2."""
    if not COPILOT_V2_ENABLED:
        return response.json(
            {
                "error": "copilot_v2_disabled",
                "message": "Copilot v2 is not enabled on this server.",
                "fallback": COPILOT_V1_API_PATH,
            },
            status=503,
        )
    ...
    try:
        ...
        from rasa.builder.copilot.pydantic_ai.agent import PydanticAICopilot
        from rasa.builder.copilot.pydantic_ai.api import (
            encode_sse_event,
            stream_copilot_v2_sse,
        )
    except ImportError:
        return response.json(
            {
                "error": "copilot_v2_missing_dependency",
                "message": (
                    "Copilot v2 requires pydantic-ai-slim. "
    ...
    sse = await request.respond(content_type="text/event-stream")
    try:
        payload: Any = request.json if request.json is not None else {}
        if not isinstance(payload, dict):
            payload = {}

        runtime = getattr(request.app.ctx, "pydantic_ai_runtime", None)
        if runtime is None:
            runtime = PydanticAICopilot(
                thread_store=_get_copilot_v2_thread_store(request)
            )
            request.app.ctx.pydantic_ai_runtime = runtime
        project_generator = get_project_generator(request)
        email = email_from_auth(request) or DEFAULT_BOT_BUILDER_EMAIL

        async for frame in stream_copilot_v2_sse(
            runtime,
            payload,
            project_generator=project_generator,
            email=email,
        ...
        structlogger.exception(
            "bot_builder_service.copilot_v2.error",
            error=str(exc),
        )
        try:
            await sse.send(
...
def _get_copilot_v2_thread_store(request: Request) -> "SQLiteThreadStore":
    """Return the per-app Copilot v2 thread store, creating it on first use.

    Shares the v1 history SQLite file (separate ``thread_messages`` table) and is
    cached on ``app.ctx`` so the chat and history endpoints read the same store.
    """
    from rasa.builder.copilot.pydantic_ai.thread_store import SQLiteThreadStore

    store = getattr(request.app.ctx, "copilot_v2_thread_store", None)
    if store is None:
        store = SQLiteThreadStore(builder_config.COPILOT_HISTORY_SQLITE_PATH)
        request.app.ctx.copilot_v2_thread_store = store
    return store


@bp.route("/copilot/v2/threads/<thread_id>/messages", methods=["GET"])
@openapi.summary("Copilot v2 thread history")
@openapi.description(
    "Return the persisted AG-UI messages for a Copilot v2 thread in insertion "
    "order. Disabled by default; set COPILOT_V2_ENABLED=true to enable."
)
@openapi.tag("copilot")
@openapi.response(
    200,
    {"application/json": {str: Any}},
    description="JSON object with `threadId` and `messages` (AG-UI message list).",
)
@openapi.response(
    503,
    {"application/json": {str: Any}},
...
@protected()
async def copilot_v2_thread_messages(request: Request, thread_id: str) -> HTTPResponse:
    """Return stored Copilot v2 messages for a thread."""
    if not COPILOT_V2_ENABLED:
        return response.json(
            {
                "error": "copilot_v2_disabled",
    ...
    store = _get_copilot_v2_thread_store(request)
    messages = await store.get_messages(thread_id)
    return response.json({"threadId": thread_id, "messages": messages})


@bp.route("/copilot/v2/models", methods=["GET"])
@openapi.summary("Copilot v2 model menu")
@openapi.description(
    "Return the models the Copilot v2 picker may offer, so the frontend renders "
    "its menu from the backend registry instead of a hardcoded copy. Disabled by "
    "default; set COPILOT_V2_ENABLED=true to enable."
)
@openapi.tag("copilot")
@openapi.response(
    200,
    {"application/json": {str: Any}},
    description=(
        "JSON object with `models`: a list of `{id, label, provider, "
        "supportsReasoning, default}` in menu order."
    ),
)
@openapi.response(
    503,
...
@protected()
async def copilot_v2_models(request: Request) -> HTTPResponse:
    """Return the Copilot v2 model-picker menu."""
    if not COPILOT_V2_ENABLED:
        return response.json(
            {
                "error": "copilot_v2_disabled",
    ...
    # Import locally: the pydantic-ai extra is optional, so keep it out of
    # this module's import-time dependencies (mirrors the SSE endpoint).
    from rasa.builder.copilot.pydantic_ai.config import PydanticAIConfig
    from rasa.builder.copilot.pydantic_ai.model_registry import list_models

    return response.json({"models": list_models(PydanticAIConfig().model_name)})
...
async def _handle_copilot_exception(
    ...
    *,
    ...
    capture_exception_with_context(
        ...
        tags={"endpoint": COPILOT_V1_API_PATH},
    )
