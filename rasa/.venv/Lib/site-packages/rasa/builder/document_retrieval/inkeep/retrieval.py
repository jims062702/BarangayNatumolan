...
from rasa.builder.document_retrieval.inkeep.constants import (
...
class InKeepDocumentRetrieval(BaseRetriever):
    ...
    async def retrieve(
        ...
        *,
        ...
        """Retrieve via the ``BaseRetriever`` contract (fallible: never raises).

        ``filters`` are ignored — InKeep is a hosted black box with no filter
        controls; ``top_k`` is applied client-side by truncating the results.
        """
        try:
        ...
        return RetrievalResponse.of(results)
