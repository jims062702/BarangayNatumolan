...
class ScenarioValidationResponse(BaseModel):
    """Response from scenario file validation."""

    success: bool = Field(description=SUCCESS_FIELD_DESCRIPTION)
    errors: List[str] = Field(
        default_factory=list, description="Validation errors found in the scenario"
    )
    message: str = Field(description="Validation status message")


class EvaluateAgentResponse(BaseModel):
    """Response from the evaluate_agent tool."""

    success: bool = Field(
        description=(
            "True when the evaluation completed without infrastructure errors "
            "(config, LLM setup, scenario loading, timeout). "
            "Does NOT indicate that all runs passed — check runs_passed vs "
            "runs_total for the outcome. False only when the tool itself failed "
            "to execute."
        )
    )
    experiment_timestamp: Optional[str] = Field(
        default=None, description="Experiment identifier used for the result files"
    )
    scenario_name: Optional[str] = Field(
        default=None, description="Name of the evaluated scenario"
    )
    runs_total: Optional[int] = Field(
        default=None, description="Total number of runs executed"
    )
    runs_passed: Optional[int] = Field(
        default=None,
        description="Number of runs that passed all assertions and criteria",
    )
    summary_path: Optional[str] = Field(
        default=None, description="Path to the experiment summary file"
    )
