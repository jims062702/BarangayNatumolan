...
from rasa.dialogue_understanding.patterns.agent_still_processing import (
    FLOW_PATTERN_AGENT_STILL_PROCESSING,
)
...
async def advance_flows(
    ...
    Args:
        ...
        session_expiration_time: Session expiration window in minutes.
...
async def advance_flows_until_next_action(
    ...
    while isinstance(step_result, ContinueFlowWithNextStep):
        ...
        with bound_contextvars(flow_id=active_frame.flow_id):
            ...
            current_step = active_frame.step(flows)
            if current_step is None:
                structlogger.warning(
                    "flow.execution.stale_step_id",
                    flow_id=active_frame.flow_id,
                    step_id=active_frame.step_id,
                    event_info=(
                        "The dialogue stack contains a frame with a step ID that no "
                        "longer exists in the flow. This typically occurs after "
                        "upgrading Rasa Pro when flow step IDs changed. The stale "
                        "frame will be removed and processing will continue."
                    ),
                )
                clean_frames = [
                    f
                    for f in tracker.stack.frames
                    if f.frame_id != active_frame.frame_id
                ]
                tracker.update_stack(DialogueStack(frames=clean_frames))
                step_result = ContinueFlowWithNextStep()
                continue
            next_step = select_next_step(
                current_step, current_flow, tracker.stack, tracker
            )
            if not next_step:
...
async def run_step(
    ...
    if isinstance(step, CollectInformationFlowStep):
        ...
        if (
            ...
            and tracker.has_session_started_for_current_message(session_expiration_time)
        ):
            structlogger.debug(
...
def _run_end_step(
    ...
    # if we are finishing flow which will notify the user while agent is running
    # we need to pop that flow from stack and pause execution,
    # so that agent stack frame stays on top
    if flow.id == FLOW_PATTERN_AGENT_STILL_PROCESSING:
        return PauseFlowReturnPrediction(
            action_prediction=FlowActionPrediction(
                action_name=ACTION_LISTEN_NAME, score=1, events=initial_events
            )
        )
...
def _set_dtmf_state_if_available(step: CollectInformationFlowStep) -> None:
    ...
    try:
        ...
        call_state.dtmf_slot = step.collect
