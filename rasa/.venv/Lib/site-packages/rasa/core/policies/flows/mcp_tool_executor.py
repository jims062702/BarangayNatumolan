...
from rasa.dialogue_understanding.patterns.internal_error import (
    ...
    INTERNAL_ERROR_SOURCE_PRE_CALL_HOOK,
...
async def _execute_mcp_tool_call(
    ...
    try:
        ...
        mapping_output = step.mapping.get("output")
        if mapping_output is None:
            return _handle_mcp_tool_error(
                stack,
                initial_events,
                f"No output mapping found for tool '{step.call}'.",
                tool_name=step.call,
                mcp_server=step.mcp_server,
                flow_id=step.flow_id,
                step_id=step.id,
        ...
        arguments = _prepare_tool_arguments(step.mapping.get("input", []), tracker)

        # Build _meta from meta_map config (not visible to the LLM)
        meta = _build_meta_for_flow_tool_call(step.mcp_server, tracker)

        # Invoke the call-time credential hook configured on the MCP server in
        # endpoints.yml, if any, and merge its returned metadata on top of the
        # static _meta. Fail-fast: if the hook fails we route to the internal
        # error pattern and never call the tool without the requested
        # credentials.
        pre_call_hook = _get_pre_call_hook_for_server(step.mcp_server)
        if pre_call_hook:
            try:
                hook = load_call_hook(pre_call_hook)
                context = _build_mcp_outbound_call_context_for_flow(
                    step=step, tracker=tracker
                )
                hook_result = await invoke_mcp_call_hook(hook, context)
                ...
                return _handle_mcp_tool_error(
                    ...
                    error_source=INTERNAL_ERROR_SOURCE_PRE_CALL_HOOK,
                    log_event="call_mcp_tool.pre_call_hook.error",
                )
        ...
        if result is None or result.isError:
            ...
            if set_slot_event := _process_tool_result(result, mapping_output):
...
def _get_pre_call_hook_for_server(mcp_server_name: Optional[str]) -> Optional[str]:
    """Get the pre_call_hook import path for an MCP server from endpoints."""
    if not mcp_server_name:
        return None
    endpoints = Configuration.get_instance().endpoints
    if not endpoints.mcp_servers:
    ...
    for server in endpoints.mcp_servers:
        if server.name == mcp_server_name:
            return server.pre_call_hook
    return None


def _build_mcp_outbound_call_context_for_flow(
    step: CallFlowStep, tracker: DialogueStateTracker
) -> MCPOutboundCallContext:
    """Build an ``MCPOutboundCallContext`` for a flow direct MCP tool call."""
...
def _prepare_tool_arguments(
    ...
    if not isinstance(input_mapping, list):
        structlogger.warning(
            "call_mcp_tool.input_mapping.not_a_list",
    ...
    for argument in input_mapping:
        if not isinstance(argument, dict):
            structlogger.warning(
                "call_mcp_tool.input_mapping.invalid_entry",
                entry_type=type(argument).__name__,
            )
            continue
        slot_name = argument.get(CONFIG_SLOT)
        param_name = argument.get("param")
        if slot_name is None or param_name is None:
            structlogger.warning(
                "call_mcp_tool.input_mapping.invalid_entry",
                slot_name=slot_name,
        ...
        arguments[param_name] = slot_value
    return arguments
...
def _process_tool_result(
    ...
    if not isinstance(output_mapping, list):
        structlogger.warning(
            "call_mcp_tool.output_mapping.not_a_list",
    ...
    try:
        ...
        for mapping in output_mapping:
            if not isinstance(mapping, dict):
                structlogger.warning(
                    "call_mcp_tool.output_mapping.invalid_entry",
                    entry_type=type(mapping).__name__,
                )
                continue
            slot_name = mapping.get(CONFIG_SLOT)
            result_expression = mapping.get(CONFIG_VALUE)
            if slot_name is None or result_expression is None:
                structlogger.warning(
                    "call_mcp_tool.output_mapping.invalid_entry",
                    slot=slot_name,
            ...
            try:
                ...
                slots.append(SlotSet(slot_name, _jsonify_slot_value(slot_value)))
            except Exception as e:
                structlogger.error(
...
def _handle_mcp_tool_error(
    ...
    error_source: str = INTERNAL_ERROR_SOURCE_MCP_TOOL,
    log_event: str = "call_mcp_tool.error",
) -> FlowStepResult:
    """Handle MCP tool or pre-call hook errors consistently.
    ...
    specific failure, then routes to ``pattern_internal_error``.
    """
    ...
    if flow_id is not None:
        log_kwargs["flow_id"] = flow_id
    if step_id is not None:
        log_kwargs["step_id"] = step_id
    if pre_call_hook is not None:
        log_kwargs["pre_call_hook"] = pre_call_hook
    structlogger.error(log_event, **log_kwargs)
    ...
    if pre_call_hook is not None:
        error_info["pre_call_hook"] = pre_call_hook
    if flow_id is not None:
