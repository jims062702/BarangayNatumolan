...
class EnterpriseSearchPolicy(LLMHealthCheckMixin, EmbeddingsHealthCheckMixin, Policy):
    ...
    @classmethod
    def _create_plain_embedder(
        cls, embeddings_config: Dict[Text, Any]
    ) -> "EmbeddingClient":
    ...
    def _return_text_response(
        ...
        Args:
            domain: The domain.
            tracker: The tracker containing the conversation history.
    ...
    def _get_local_knowledge_data(
        ...
        docs_as_strings = [
            json.dumps(doc.model_dump(), ensure_ascii=False, sort_keys=True)
            for doc in docs
        ]
    ...
    def post_process_citations(llm_answer: str) -> str:
        """Renumbers and sorts citations and sources in the LLM answer.
    ...
    def _get_cited_order(
        ...
        """Find unique citations in the answer text, in order of first appearance."""
    ...
    def _build_final_sources_list(
        ...
        """Build the final list of sources in cited order with renumbered indices."""
    ...
    def get_system_default_prompt_based_on_config(cls, config: Dict[str, Any]) -> str:
        """Resolves the default prompt template based on the component's configuration.
