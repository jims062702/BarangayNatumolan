...
def truncate_documents(
    ...
    docs_token_num = 0
    for doc in docs:
        doc_token_num = len(encode(doc.text))
...
class IntentlessPolicy(LLMHealthCheckMixin, EmbeddingsHealthCheckMixin, Policy):
    ...
    def __init__(
        ...
        responses_docsearch: Optional["FAISS_Store"] = None,
        samples_docsearch: Optional["FAISS_Store"] = None,
    ...
    @classmethod
    def _create_plain_embedder(cls, config: Dict[Text, Any]) -> EmbeddingClient:
    ...
    def embed_llm_response(self, llm_response: str) -> Optional[List[float]]:
        ...
        try:
            ...
            return embed_documents(self.embedder, [llm_response])[0]
        except Exception as e:
            # unfortunately, langchain does not wrap LLM exceptions which means
            # we have to catch all exceptions here
    ...
    def select_response_examples(
        ...
        try:
            embedding = embed_query(self.embedder, history)
        except Exception as e:
            structlogger.error(
        ...
        **kwargs: Any,
        ...
        try:
            with model_storage.read_from(resource) as path:
                ...
                # The `normalize_l2` flag is now persisted on disk in
                # `store.json`, so the previous post-load monkey-patch that set
                # `responses_docsearch._normalize_L2 = True` is no longer needed.
