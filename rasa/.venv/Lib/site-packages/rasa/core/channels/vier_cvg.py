...
def _required_recipient_field(value: Any, field_name: Text) -> str:
    if value is None:
        raise ValueError(f"Invalid recipient_id payload: missing '{field_name}'")

    text_value = str(value).strip()
    if not text_value:
        raise ValueError(f"Invalid recipient_id payload: empty '{field_name}'")
...
def parse_recipient_id(recipient_id: Text) -> Recipient:
    try:
        decoded = base64.b64decode(bytes(recipient_id, "utf-8")).decode("utf-8")
        ...
        raise ValueError("Invalid recipient_id") from error

    if not isinstance(parsed_json, dict):
        raise ValueError("Invalid recipient_id payload")

    project_context = parsed_json.get("projectContext")
    if not isinstance(project_context, dict):
        raise ValueError("Invalid recipient_id payload")

    return Recipient(
        dialog_id=_required_recipient_field(parsed_json.get("dialogId"), "dialogId"),
        project_token=_required_recipient_field(
            project_context.get("projectToken"), "projectContext.projectToken"
        ),
        reseller_token=_required_recipient_field(
            project_context.get("resellerToken"), "projectContext.resellerToken"
        ),
    )
...
class CVGOutput(OutputChannel):
    ...
    async def _execute_operation_by_name(
        ...
        async def handle_outbound_call_result(
            ...
            logger.info(
                "Creating incoming UserMessage: {output_channel=%s, dialog_id=%s}"
                % (
    ...
    async def process_message(
        ...
        try:
            ...
            if sender_id is not None:
                try:
                    dialog_id = parse_recipient_id(sender_id).dialog_id
                except ValueError:
                    pass
