...
def _form_field_value(form: Dict[Text, Any], field: Text) -> Optional[Text]:
    """Return a single value for a URL-encoded form field."""
...
def _first_interactive_action(payload: Dict[Text, Any]) -> Optional[Dict[Text, Any]]:
    """Return the first Slack interactive action dict, if present and well-formed."""
...
class SlackInput(InputChannel):
    ...
    @staticmethod
    def _event_user_id(event: Dict[Text, Any]) -> Text:
        """Return the Slack user id from an event payload, if present."""
        user = event.get("user", "")
    ...
    def _is_interactive_message(payload: Dict) -> bool:
        ...
        if payload.get("actions"):
            action = _first_interactive_action(payload)
            if action is None:
                return False
    ...
    def _get_interactive_response(action: Dict) -> Optional[Text]:
        ...
        if action_type == "button":
            ...
            return action.get("selected_option", {}).get("value")
        elif action_type == "conversations_select":
    ...
    def get_metadata(self, request: Request) -> Dict[Text, Any]:
        ...
        if content_type == "application/x-www-form-urlencoded":
            ...
            output = request.form
            payload_raw = _form_field_value(output, "payload")
            if not payload_raw:
                return {}
    ...
    def blueprint(
        ...
        async def webhook(request: Request) -> HTTPResponse:
            ...
            if content_type == "application/json":
                ...
                sender_id = self._event_user_id(event)
                if not sender_id:
                    return response.text(
                        "Missing user id.",
                        status=HTTPStatus.BAD_REQUEST,
                ...
                if self._is_interactive_message(payload):
                    ...
                    if action is None:
                        return response.text(
                            "Missing interactive action.",
                            status=HTTPStatus.BAD_REQUEST,
                        )
                    text = self._get_interactive_response(action)
                    if text is not None:
                        if not sender_id:
                            return response.text(
                                "Missing user id.",
