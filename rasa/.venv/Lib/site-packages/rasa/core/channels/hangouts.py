...
class HangoutsOutput(OutputChannel):
    ...
    async def _persist_message(self, message: Dict) -> None:
        """Send a message to Google Hangouts.

        Only accepts single dict with single key 'text' for simple text messages;
        all other responses must be sent as cards. When the bot sends multiple
        messages, all are transformed to either cards or text output.
        """
    ...
    async def send_custom_json(
        ...
        """Forward a custom JSON payload to Google Hangouts without modification.

        Use this for more complex cards, which can be created in actions.py.
        """
