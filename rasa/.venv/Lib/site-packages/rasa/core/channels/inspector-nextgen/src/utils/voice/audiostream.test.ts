...
describe("audiostream", () => {
  describe("utility functions", () => {
    ...
    it("converts Int16Array to Float32Array using MAX_INT16_VALUE", () => {
      ...
      const floatArray = Float32Array.from(intArray, (x) => x / MAX_INT16_VALUE);
  ...
  describe("setupAudioPlayback", () => {
    ...
    it("creates AudioQueue with zero queuedSamples and empty marks", async () => {
      const audioQueue = await setupAudioPlayback(mockSocket, mockLogError, 48000);
    ...
    it("resumes suspended audio context", async () => {
      ...
      globalThis.AudioContext = vi.fn().mockReturnValue(suspendedContext) as never;
    ...
    it("does not resume running audio context", async () => {
      ...
      globalThis.AudioContext = vi.fn().mockReturnValue(runningContext) as never;
  ...
  describe("streamMicrophoneToServer", () => {
    ...
    it("creates AudioContext with correct sample rate", async () => {
      ...
      vi.mocked(navigator.mediaDevices.getUserMedia).mockResolvedValue(mockStream as never);
    ...
    it("throws error when microphone access is denied", async () => {
      ...
      await expect(streamMicrophoneToServer(mockSocket, mockLogError, 48000)).rejects.toThrow(
        "Permission denied",
      );
    });
    ...
    it("returns MicrophoneStream object on success", async () => {
      ...
      const result = await streamMicrophoneToServer(mockSocket, mockLogError, 48000);
  ...
  describe("stopMicrophoneStream", () => {
    ...
    it("stops all media tracks", async () => {
      ...
      const microphoneStream = await streamMicrophoneToServer(mockSocket, mockLogError, 48000);
    ...
    it("closes audio context if not already closed", async () => {
      ...
      const microphoneStream = await streamMicrophoneToServer(mockSocket, mockLogError, 48000);
      const audioContext = microphoneStream?.audioContext as unknown as MockAudioContext;
  ...
  describe("addDataToAudioQueue", () => {
    ...
    describe("audio data", () => {
      it("adds audio data to queue", () => {
        ...
        const MAX_INT16_VALUE = 0x7fff;
        const intArray = Int16Array.from(audioData, (x) => x * MAX_INT16_VALUE);
      ...
      it("throws on invalid base64 audio data", () => {
        expect(() => addData(JSON.stringify({ audio: "invalid-base64", extra: "data" }))).toThrow();
      });
    });
    ...
    describe("error handling", () => {
      ...
      it("throws on non-object parsed data", () => {
        expect(() => addData(JSON.stringify("string value"))).toThrow("Invalid message format");
      });

      it("throws on null parsed data", () => {
        expect(() => addData(JSON.stringify(null))).toThrow("Invalid message format");
      });

      it("throws on error property in data", () => {
        expect(() => addData(JSON.stringify({ error: "Something went wrong" }))).toThrow(
      ...
      it("throws on empty object", () => {
        expect(() => addData(JSON.stringify({}))).toThrow("Unknown data structure");
      });
    });
  });
});
