...
Repository pre-commit hooks run Prettier (`lint-staged`), `yarn lint`, `yarn test`, and `yarn build` here; use **`nvm use`** (if applicable), then `yarn install`, so those commands run on Node 24 with Yarn 4.

### Code formatting

Formatting uses [Prettier](https://prettier.io/) with project settings in [`.prettierrc`](.prettierrc). The same file types are formatted on commit, locally, and in CI (aligned with the hello repo `lint-staged` pattern): **json, html, css, yml, js, jsx, ts, tsx**.

**`.prettierignore`** skips build output and docs: `dist`, `*.md`, and generated test reports (`coverage`, `playwright-report`, `e2e/playwright-report`).

- **Editor:** enable format on save and use this repo’s Prettier config (avoid a global Prettier config that overrides `.prettierrc`).
- **Fix all:** `yarn format`
- **Check only (same as CI):** `yarn format:check`
- **Lint:** `yarn lint` (code quality; run after formatting if both changed)

CI runs `yarn format:check` in the `inspector-lint` job. If it fails, run `yarn format` in this directory and commit the changes.
