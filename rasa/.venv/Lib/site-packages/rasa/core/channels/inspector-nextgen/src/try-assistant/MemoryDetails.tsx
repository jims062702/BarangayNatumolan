...
export const SlotsDetails = () => {
  ...
  // Current flow slots
  const currentFlowSlots = formatSlots(currentFlowSlotEvents).filter((slot) => slot.value != null);
  ...
  const eventBasedSystemSlots = formatSlots(slotRelatedEvents).filter((slot) =>
    isSystemSlotEvent(slot.name),
  );
  const coveredNames = new Set([
    ...currentFlowSlots.map((s) => s.name),
    ...sessionSlots.map((s) => s.name),
    ...eventBasedSystemSlots.map((s) => s.name),
  ]);
  const trackerOnlySystemSlots: SlotState[] = Object.entries(slots)
    .filter(([name, value]) => isSystemSlotEvent(name) && !coveredNames.has(name) && value != null)
    .map(([name, value]) => ({ name, value }));

  // System slots
  const systemSlots = [...eventBasedSystemSlots, ...trackerOnlySystemSlots];

  const sections = [
    ...(currentFlowSlots.length > 0 ? [{ title: "Current flow", slots: currentFlowSlots }] : []),
    ...(sessionSlots.length > 0 ? [{ title: "Session", slots: sessionSlots }] : []),
    ...(systemSlots.length > 0 ? [{ title: "System", slots: systemSlots }] : []),
  ];
    ...
    <Box bg="white" borderRadius="lg">
      <ScrollContainer>
        <ScrollContent
          ...
          pr="7"
          py="2"
          css={{ "& [data-orientation='vertical']": { display: "none" } }}
        >
  ...
  return (
    <Box display="flex" flexDirection="column" gap="4">
      {sections.map((section, index) => (
        <Box key={index}>
          {index > 0 && <Separator mb="4" mx="-4" />}
          <Box display="flex" flexDirection="column" data-testid={`${section.title}-section`}>
            <Heading textStyle="sm" px="2" py="1" mb="1">
              {section.title}
            </Heading>
            {section.slots.map((slot) => (
              <SlotRow key={slot.name} slot={slot} isActive={slot.name === activeSlotName} />
            ))}
          </Box>
        </Box>
      ))}
...
const SlotRow = ({ slot, isActive }: { slot: SlotState; isActive: boolean }) => {
  const { event } = slot;
  ...
  return (
    <Box
      ...
      color="fg"
      _hover={event ? { bg: "bg.muted", cursor: "pointer" } : undefined}
      onClick={event ? () => toggleSelectedElement(event) : undefined}
      data-testid={`slot-${slot.name}`}
    >
      <Text variant="primary" textStyle="sm" width="50%" truncate title={slot.name}>
        {slot.name}
      </Text>
      <Text
        variant="primary"
