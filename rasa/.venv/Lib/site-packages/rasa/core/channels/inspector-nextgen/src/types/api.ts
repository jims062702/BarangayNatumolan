...
import type { NodeConditionSubType, NodeType } from ".";
...
export function isFlowStep(next: FlowStep | FlowStepCondition): next is FlowStep {
  return (next as FlowStep).stepType !== undefined;
}

export function isFlowStepThen(next: FlowStepCondition | FlowStep): next is FlowStepThen {
  return (next as FlowStepThen).if !== undefined && (next as FlowStepThen).then !== undefined;
}

export function isFlowStepElse(next: FlowStepCondition | FlowStep): next is FlowStepElse {
...
const baseStepSchema = z
  .object({
    ...
    link: z.string(),
    set_slots: z.array(
      z.record(z.string(), z.union([z.string(), z.boolean(), z.number(), z.null()])),
    ),
    call: z.string(),
    id: z.string(),
    utter: z.string(),
...
type Step = z.infer<typeof baseStepSchema> & {
  next?: Step[] | string | ({ if: string; then: Step[] | string } | { else: Step[] | string })[];
};
...
const stepSchema: z.ZodType<Step> = baseStepSchema
  ...
  .refine(
    (step) =>
      (step.noop && !step.collect && !step.action && !step.link && !step.set_slots && !step.call) ||
      (step.collect && !step.action && !step.link && !step.set_slots && !step.call && !step.noop) ||
      (step.action && !step.collect && !step.link && !step.set_slots && !step.call && !step.noop) ||
      (step.set_slots && !step.collect && !step.action && !step.link && !step.call && !step.noop) ||
      (step.link && !step.collect && !step.action && !step.set_slots && !step.call && !step.noop) ||
      (step.call && !step.collect && !step.action && !step.link && !step.set_slots && !step.noop),
  )
  .transform((step) => {
