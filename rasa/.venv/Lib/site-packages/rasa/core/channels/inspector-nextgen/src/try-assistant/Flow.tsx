...
function findShortestPath(edges: FlowEdge[], start: string, end: string): string[] | null {
  ...
  while (queue.length > 0) {
    ...
    if (!visited.has(node)) {
      ...
      const neighbors = edges.filter((edge) => edge.sourceId === node).map((edge) => edge.targetId);
...
function findSourceNode(nodes: FlowNode[], edge: FlowEdge): FlowNode | undefined {
  return nodes.find((node) => node.id === edge.sourceId);
}

function findTargetNode(nodes: FlowNode[], edge: FlowEdge): FlowNode | undefined {
...
function collectHighlightedEdges(
  ...
  const fullTrail: string[] = flowStepTrail(events)[flow.id || ""] || [];
  ...
  let previousHighlightedStep: string | null = fullTrail.length > 0 ? fullTrail[0] : null;

  for (let i = 0; i < fullTrail.length - 1; i++) {
    const currentStep = fullTrail[i];
    const nextStep = fullTrail[i + 1];
    ...
    if (!path) {
      ...
      // the current step
      path = findShortestPath(flow.edges || [], previousHighlightedStep, nextStep);
      if (!path) {
    ...
    for (let j = 0; j < path.length - 1; j++) {
      const edge = flow.edges?.find((e) => e.sourceId === path[j] && e.targetId === path[j + 1]);
      if (edge) {
  ...
  const activeNodeId =
    selectedNodeId && fullTrail.includes(selectedNodeId)
      ? selectedNodeId
      : (fullTrail.at(-1) ?? null);
...
export function Flow({ flow, selectedNodeId, conversation }: Props) {
  const { highlightedEdges, highlightedNodes, activeNodeId } = useMemo(() => {
    if (!flow) {
      return { highlightedEdges: [], highlightedNodes: [], activeNodeId: null };
    }
    return collectHighlightedEdges(conversation?.events ?? [], flow, selectedNodeId);
  }, [conversation?.events, flow, selectedNodeId]);
