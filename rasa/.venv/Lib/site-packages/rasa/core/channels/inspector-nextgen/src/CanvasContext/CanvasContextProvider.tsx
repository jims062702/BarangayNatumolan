import { debounce } from "lodash";
import { type ReactNode, useCallback, useEffect, useMemo, useRef, useState } from "react";
import {
  type Edge,
  type Node,
  type ReactFlowInstance,
  MarkerType,
  useEdgesState,
  useNodesState,
  useReactFlow,
} from "reactflow";
import { DEFAULT_NODE_MAX_HEIGHT, DEFAULT_NODE_WIDTH } from "../constants";
import { useTheme } from "../hooks/useTheme";
import { useInspectorContext } from "../InspectorContext";
import {
  type CanvasNodeData,
  type FlowEdge,
  type FlowNode,
  FlowEdgeType,
  FlowNodeType,
  NodeState,
} from "../types";
import { autoLayout } from "../utils";
import { findFocusNode } from "../utils/canvas";
import { Context } from "./CanvasContext";

interface Props {
  flowName: string;
  initNodes: FlowNode[];
  initEdges: FlowEdge[];
  highlightedEdges?: FlowEdge[];
  highlightedNodes?: FlowNode[];
  activeNodeId?: string | null;
  children: ReactNode;
}

export const CanvasContextProvider = ({
  children,
  flowName,
  initNodes = [],
  initEdges = [],
  highlightedEdges = [],
  highlightedNodes = [],
  activeNodeId = null,
}: Props) => {
  const { logError } = useInspectorContext();
  const { getToken, getTokenPx } = useTheme();
  const defaultEdgeColor = getToken("colors.gray.300");
  const highlightedEdgeColor = getToken("colors.gray.600");
  const edgeBorderRadius = parseInt(getToken("radii.lg"));
  const markerSize = getTokenPx("sizes.5");
  const { zoomIn, zoomOut, fitView, zoomTo } = useReactFlow();
  const [nodesWithCoordinates, setNodesWithCoordinates] = useState([] as FlowNode[]);
  const [flowInstance, setFlowInstance] = useState<ReactFlowInstance | null>(null);
  const [nodes, setNodes, handleNodesChange] = useNodesState(
    nodesApiToCanvas(nodesWithCoordinates, initEdges, highlightedNodes, activeNodeId),
  );
  const [edges, setEdges] = useEdgesState(
    edgesApiToCanvas(
      initEdges,
      highlightedEdges,
      defaultEdgeColor,
      highlightedEdgeColor,
      edgeBorderRadius,
      markerSize,
    ),
  );

  const focusNode = useMemo(() => findFocusNode(nodes, activeNodeId), [nodes, activeNodeId]);

  useEffect(() => {
    setEdges(
      edgesApiToCanvas(
        initEdges,
        highlightedEdges,
        defaultEdgeColor,
        highlightedEdgeColor,
        edgeBorderRadius,
        markerSize,
      ),
    );
  }, [
    initEdges,
    highlightedEdges,
    setEdges,
    defaultEdgeColor,
    highlightedEdgeColor,
    edgeBorderRadius,
    markerSize,
  ]);

  const initializeNodes = useCallback(
    (initNodes: FlowNode[], initEdges: FlowEdge[]) => {
      autoLayout(initNodes, initEdges)
        .then((nodesWithCoords) => setNodesWithCoordinates(nodesWithCoords))
        .catch((error) => {
          logError(error, {
            tags: {
              component: "CanvasContextProvider",
              action: "initializeNodes",
            },
          });
        });
    },
    [setNodesWithCoordinates, logError],
  );

  useEffect(() => {
    initializeNodes(initNodes, initEdges);
  }, [initNodes, initEdges, initializeNodes]);

  useEffect(() => {
    if (nodesWithCoordinates.length > 0) {
      const newNodes = nodesApiToCanvas(
        nodesWithCoordinates,
        initEdges,
        highlightedNodes,
        activeNodeId,
      );
      setNodes(newNodes);
    }
  }, [nodesWithCoordinates, initEdges, setNodes, highlightedNodes, activeNodeId]);

  const focusOnNode = useCallback(
    (node: Node | undefined | null = null, animate?: boolean) => {
      if (!flowInstance || !node) {
        return;
      }

      const { zoom } = flowInstance.getViewport();
      const duration = animate ? 200 : 0;
      const x = node.position.x + DEFAULT_NODE_WIDTH / 2;
      const y = node.position.y + DEFAULT_NODE_MAX_HEIGHT / 2;

      flowInstance.setCenter(x, y, { zoom, duration });
    },
    [flowInstance],
  );

  useEffect(() => {
    const debouncedResize = debounce(function resize() {
      focusOnNode(focusNode ?? nodes[0], true);
    }, 500);

    globalThis.addEventListener("resize", debouncedResize);

    return () => {
      globalThis.removeEventListener("resize", debouncedResize);
    };
  }, [focusNode, nodes, focusOnNode]);

  const timeoutRef = useRef<ReturnType<typeof globalThis.setTimeout>>(null);

  useEffect(() => {
    timeoutRef.current = setTimeout(() => (focusNode ? focusOnNode(focusNode, true) : null), 0);

    return () => {
      if (timeoutRef.current) clearTimeout(timeoutRef.current);
    };
  }, [focusOnNode, focusNode]);

  const handleZoomInClick = () => {
    zoomIn({ duration: 200 });
  };

  const handleZoomOutClick = () => {
    zoomOut({ duration: 200 });
  };

  const handleZoomTo100 = () => {
    zoomTo(1, { duration: 200 });
  };

  const handleFitToCanvasClick = () => {
    fitView({ minZoom: 0.1 });
  };

  const handleInit = useCallback(
    (instance: ReactFlowInstance) => {
      setFlowInstance(instance);
      focusOnNode(instance.getNodes()?.[0], false);
    },
    [setFlowInstance, focusOnNode],
  );

  const handleResize = useCallback(() => {
    if (flowInstance) {
      focusOnNode(flowInstance.getNodes()?.[0], false);
    }
  }, [flowInstance, focusOnNode]);

  const value = {
    flowName,
    nodes,
    edges,
    handleInit,
    handleResize,
    focusOnNode,
    handleNodesChange,
    handleZoomInClick,
    handleZoomOutClick,
    handleZoomTo100,
    handleFitToCanvasClick,
  };

  return <Context.Provider value={value}> {children} </Context.Provider>;
};

function nodesApiToCanvas(
  nodes: FlowNode[],
  edges: FlowEdge[],
  highlightedNodes: FlowNode[],
  activeNodeId: string | null,
  isReadOnly: boolean = true,
): Node<CanvasNodeData>[] {
  const startNodeType = nodes.length === 1 ? "startAlone" : "start";

  return nodes.map((node) => {
    const outboundEdges = edges.filter(({ sourceId }) => sourceId === node.id);
    const isLeafNode = outboundEdges.length === 0;
    const isStartNode = node.type === FlowNodeType.Start;
    const isActive = node.id === activeNodeId;
    const isHighlighted =
      !isActive && highlightedNodes?.some((highlightedNode) => highlightedNode.id === node.id);

    let nodeState: NodeState;
    if (isActive) {
      nodeState = NodeState.Active;
    } else if (isHighlighted) {
      nodeState = NodeState.Passed;
    } else {
      nodeState = NodeState.Upcoming;
    }

    let data: FlowNode;

    switch (node.type) {
      case FlowNodeType.Start:
        data = { ...node };
        break;
      case FlowNodeType.CollectInformation:
        data = { ...node };
        break;
      case FlowNodeType.CustomAction:
        data = { ...node };
        break;
      case FlowNodeType.Logic:
        data = { ...node };
        break;
      case FlowNodeType.Link:
        data = { ...node };
        break;
      case FlowNodeType.Message:
        data = { ...node };
        break;
      case FlowNodeType.Condition:
        data = { ...node };
        break;
      case FlowNodeType.SetSlots:
        data = { ...node };
        break;
      case FlowNodeType.Call:
        data = { ...node };
        break;
      default:
        data = { ...(node as FlowNode) };
    }

    return {
      id: node.id,
      position: {
        x: node.metadata.x,
        y: node.metadata.y,
      },
      type: isStartNode ? startNodeType : isLeafNode ? "end" : "standard",
      data: {
        ...data,
        isReadOnly,
        nodeState,
      },
      selected: isActive || !!isHighlighted,
      selectable: false,
    };
  });
}

function isHighlightedEdge(edge: FlowEdge, highlightedEdges: FlowEdge[]) {
  return highlightedEdges.some(
    (highlightedEdge) =>
      highlightedEdge.sourceId === edge.sourceId && highlightedEdge.targetId === edge.targetId,
  );
}

function edgesApiToCanvas(
  edges: FlowEdge[],
  highlightedEdges: FlowEdge[],
  defaultEdgeColor: string,
  highlightedEdgeColor: string,
  edgeBorderRadius: number,
  markerSize: number,
): Edge[] {
  return edges.map((edge) => {
    const type = edge.type === FlowEdgeType.Custom ? "custom" : "add";
    const isHighlighted = isHighlightedEdge(edge, highlightedEdges);
    const edgeColor = isHighlighted ? highlightedEdgeColor : defaultEdgeColor;

    return {
      id: `${edge.sourceId}-${edge.targetId}`,
      source: edge.sourceId,
      target: edge.targetId,
      type,
      selected: isHighlighted ?? false,
      zIndex: isHighlighted ? 1 : 0,
      style: { stroke: edgeColor },
      markerEnd: {
        type: MarkerType.Arrow,
        width: markerSize,
        height: markerSize,
        color: edgeColor,
      },
      data: {
        isReadOnly: true,
      },
      pathOptions: { borderRadius: edgeBorderRadius },
    };
  });
}
