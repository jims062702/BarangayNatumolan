  ...
  if (stepType === undefined) {
    ...
    switch (key) {
      ...
      case "noop":
        ...
        break;
      default:
        throw new Error(`Cannot determine step type for step: ${JSON.stringify(step, null, 2)}`);
    }
  }
...
function stepNextIsLogic(
  ...
  return (
    typeof stepNext === "object" &&
    stepNext.length > 0 &&
    ("if" in stepNext[0] || "else" in stepNext[0])
  );
}

// Copy from Studio
export function generateLogicStepIdForStep(step: { noop?: boolean; id?: string }) {
...
export function buildFlowStructureFromSteps({
  ...
  if (step.next && step.next !== "END") {
    if (typeof step.next === "string") {
      localStepSources[step.next] = [...(localStepSources[step.next] || []), stepWithId];
      ...
      const allConditionSteps: (PreNodeFlowStep & { id: string })[] = conditionsArray.map(
        (condition, index) => ({
          ...condition,
      ...
      const hasIfEntry = conditionsArray.some(isFlowStepThen);
      if (!hasIfEntry) {
        // else-only branch: skip the If... parent node and wire directly to Else Condition
        for (const conditionStep of allConditionSteps) {
          localStepSources[conditionStep.id] = [stepWithId];
          localConditionStepSources[conditionStep.id] = stepWithId.id;
        }
        rest = [...allConditionSteps, ...rest];
      } else {
...
function stepCallType(step: PreNodeFlowStep, flowNamesList?: string[]): CallType {
...
export function createFlowNodesFromFlowSteps(
  ...
  const { steps: processedSteps, edges } = buildFlowStructureFromSteps({
  ...
  for (const step of processedSteps) {
    ...
    const node = createFlowNodeFromStep(step, flowNamesList);
    if (!step.id) {
  ...
  for (const stepId of Object.keys(edges)) {
    ...
    const sourceNodes = sourceSteps.map((sourceStep) => {
      if (sourceStep && !sourceStep.id) {
        throw new Error(`Source step should always have an ID, flow: ${flowName}`);
      }
  ...
  if (!firstNonNoopStep) {
    throw new Error(`No valid target step found for start edge, flow: ${flowName}`);
  }
