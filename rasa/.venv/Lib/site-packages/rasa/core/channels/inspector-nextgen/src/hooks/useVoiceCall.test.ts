...
describe("useVoiceCall", () => {
  ...
  describe("error handling", () => {
    it("should handle SocketTimeoutError and call stopVoiceStreaming", async () => {
      startVoiceStreaming.mockRejectedValueOnce(new SocketTimeoutError("Timeout"));
    ...
    it("should handle SocketUnavailableError", async () => {
      startVoiceStreaming.mockRejectedValueOnce(new SocketUnavailableError("Unavailable"));
    ...
    ])("should handle $name with message: $description", async ({ name, description }) => {
    ...
    it("should stop voice call and show toast when voiceError callback is invoked", async () => {
      ...
      expect(mockShowToast).toHaveBeenCalledWith({
        ...
        description:
          "To test in voice, add your Voice API keys and complete the voice configuration.",
        type: "warning",
        closable: true,
      });
  ...
  it("should guard against concurrent startVoiceCall (no interval leak)", async () => {
    ...
    await act(async () => {
      await Promise.all([result.current.startVoiceCall(), result.current.startVoiceCall()]);
    });
  ...
  it("should guard against concurrent stopVoiceCall", async () => {
    ...
    await act(async () => {
      await Promise.all([result.current.stopVoiceCall(), result.current.stopVoiceCall()]);
    });
