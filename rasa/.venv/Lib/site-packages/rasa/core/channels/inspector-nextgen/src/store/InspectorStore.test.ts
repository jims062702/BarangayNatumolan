...
import { createInspectorStore, initInspectorStore, inspectorStore } from "./InspectorStore";
...
describe("createInspectorStore", () => {
  ...
  it("default actions are safe to call (noops)", () => {
    const store = createInspectorStore();
    const { sendMessage, startNewConversation, replayConversation } = store.state;
  ...
  it("merges partial initial state over defaults", () => {
    ...
    expect(store.state.authToken).toBe("secret-token");
...
describe("initInspectorStore", () => {
  it("resets the singleton state to defaults", () => {
  ...
  it("applies initial state over defaults", () => {
