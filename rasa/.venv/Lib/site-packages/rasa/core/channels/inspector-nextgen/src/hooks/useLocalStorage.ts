...
export function useLocalStorage<T>(
  ...
  useEffect(() => {
    ...
    globalThis.addEventListener("local-storage-changed", handleLocalStorageChange as EventListener);
  ...
  const setValue = useCallback(
    (value: T | ((prev: T) => T)) => {
      ...
      try {
        ...
        console.warn(`Failed to set localStorage key "${keyRef.current}":`, error);
      }
    },
    [storedValue],
  );
