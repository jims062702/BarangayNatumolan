...
export function useBotConnection({
  ...
  const [localStorageHistory, setLocalStorageHistory] = useLocalStorage<ConversationHistory>(
  ...
  const socketReadyPromiseResolveRef = useRef<((value: void) => void) | undefined>(undefined);
  ...
  const [, setError] = useState<ModelServiceError | RasaProError | undefined>(undefined);
  ...
  const {
    ...
    trackerEndpoint: s.trackerEndpoint,
  ...
  const microphoneStreamRef =
    useRef<ReturnType<typeof streamMicrophoneToServer> extends Promise<infer T> ? T : never>(
      undefined,
    );
  ...
  const [slotRelatedEvents, setSlotRelatedEvents] = useState<ConversationEvent[]>([]);
  ...
  const sendMessage = useCallback(
    (message: string) => {
      ...
      const userUtterance: Utterance = generatePlaceholderUserUtterance(message);
  ...
  const handleVoiceSessionEnd = useCallback(() => {
    const prevMicStream = microphoneStreamRef.current;
    microphoneStreamRef.current = undefined;
    void stopMicrophoneStream(prevMicStream).catch(() => undefined);
    void (async () => {
      await audioQueueRef.current?.waitForDrain(10000);
  ...
  const handleTrackerResponse = useCallback(
    (response: TrackerResponseData) => {
      if (!response) return;

      const shouldProcessResponse = response.sender_id === sessionId;
      if (shouldProcessResponse) {
        const rawEventList = response.events ?? [];
        const lastRawEvent = rawEventList.at(-1);

        if (lastRawEvent && isStreamingFrame(lastRawEvent)) {
          // Streaming chunks are published on the same "tracker" event as live
          // tracker updates, but they carry the streaming output channel's
          // tracker snapshot, which lags the live conversation by at least one
          // turn. Rendering their full event list would replace the conversation
          // history with a previous-turn state (the current user message and any
          // completed bot turns vanish) and snap back when the stream ends -- the
          // per-turn flicker. So for a streaming frame we keep the existing
          // history and only update the live streaming bubble on top of it.
          const mappedStreaming = mapRawEventsToConversationEvents([lastRawEvent]);
          const streamingUtterance = mappedStreaming.at(-1);
          if (streamingUtterance) {
            setConversation((conv) => {
              const baseEvents = conv.events.filter((event) => !isStreamingUtterance(event));
              return { ...conv, events: [...baseEvents, streamingUtterance] };
            });
          }
          return;
        }
        ...
        const convertedStack: Stack[] = response.stack.map((item: RawStack) => ({
        ...
        setConversation((conv) => {
          return {
            ...conv,
            events,
            totalNumberOfUserMessages: events.filter((event: UnionEventType) => isUtterance(event))
              .length,
        ...
        const sessionEnded = events.some(
          (e) =>
            isConversationEvent(e) &&
            e.conversationEventType === ConversationEventType.SessionEnded,
        );
        if (sessionEnded) {
          if (activeModalityRef.current === "voice") {
    ...
    [sessionId, disableChat, handleVoiceSessionEnd],
  );
  ...
  useEffect(() => {
    ...
    if (!socket.current && url) {
          ...
          if (activeModalityRef.current === "text") {
            if (!externalSessionId) {
              ...
              getConversationHistory({
                ...
                .then((trackerResult) => handleTrackerResponse(trackerResult))
                .catch((err) => {
                  logError(err);
                });
            }
      ...
      socket.current?.io.on("reconnect_error", (error) => {
        if (!isToastVisible(RECONNECT_TOAST_ID)) {
          showToast({
      ...
      socket.current?.io.on("reconnect_failed", () => {
        ...
        updateToast(RECONNECT_TOAST_ID, {
          title: "Reconnect failed",
  ...
  const startNewConversation = useCallback((): string => {
    ...
    if (activeModalityRef.current === "voice") {
      ...
      const prevMicStream = microphoneStreamRef.current;
      const prevAudioQueue = audioQueueRef.current;
      microphoneStreamRef.current = undefined;
      audioQueueRef.current = undefined;
      void stopMicrophoneStream(prevMicStream).catch(() => undefined);
      void stopAudioPlayback(prevAudioQueue).catch(() => undefined);
    } else {
