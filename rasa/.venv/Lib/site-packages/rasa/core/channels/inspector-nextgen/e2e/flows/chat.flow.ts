...
const assertEventDetailsState = async (page: Page, expectations: EventDetailsExpectation) => {
  await ui.eventDetails.assertions(page).panelIsVisible(expectations.panelTitle);

  if (expectations.slotOrFlowName) {
    await ui.eventDetails.assertions(page).slotOrFlowNameIsVisible(expectations.slotOrFlowName);
  }
...
export const clickBotButtonAndAssertReply = async (page: Page, buttonLabel: string) => {
  const botCountBefore = await ui.conversationLog.getLocators(page).botMessages.count();

  await test.step(`Click bot button "${buttonLabel}"`, async () => {
    await ui.conversationLog.actions(page).clickBotButton(buttonLabel);
  });

  await test.step("Assert bot responds after button click", async () => {
    await ui.conversationLog.assertions(page).botMessageCountAtLeast(botCountBefore + 1);
  });
};

export const sendMessageWithEnterAndAssert = async (page: Page, message: string) => {
  const userCountBefore = await ui.conversationLog.getLocators(page).userMessages.count();
  ...
  await test.step("Assert user message in chat and input cleared", async () => {
    await ui.conversationLog.assertions(page).userMessageIsVisible(message);
    await ui.inspectorShell.assertions(page).inputIsCleared();
    await ui.conversationLog.assertions(page).userMessageCountIs(userCountBefore + 1);
  });
};
...
export const openConversationEventDetailsAndAssert = async (
  ...
  await test.step(`Click event "${expectations.eventName}"`, async () => {
    await ui.conversationLog.actions(page).clickConversationEvent(expectations.eventName);
  });

  await test.step(`Assert event details panel shows "${expectations.panelTitle}"`, async () => {
...
export const openEventDetailsWithKeyboardAndAssert = async (
  page: Page,
  eventName: string,
  panelTitle: string,
) => {
  await test.step(`Focus event "${eventName}" and press Enter`, async () => {
    const eventButton = page.getByRole("button", {
      name: eventName,
      exact: true,
    });
    await eventButton.focus();
    await eventButton.press("Enter");
  });

  await test.step(`Assert event details panel shows "${panelTitle}"`, async () => {
    await ui.eventDetails.assertions(page).panelIsVisible(panelTitle);
  });
};
...
export const clickBotMessageAndAssertNoEventDetails = async (page: Page, index = 0) => {
  await test.step(`Click bot message at index ${index}`, async () => {
    await ui.conversationLog.actions(page).clickBotMessage(index);
  });
