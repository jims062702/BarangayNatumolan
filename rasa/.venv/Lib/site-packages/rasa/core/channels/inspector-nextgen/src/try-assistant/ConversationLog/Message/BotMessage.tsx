import { Box, Button, Flex, Image, Separator, Text, useDisclosure } from "@chakra-ui/react";
import { forwardRef, useState } from "react";
import type { MessagePropsWithTopIntentName, ResponseData } from "../../../types";
...
const startsWithProtocol = (payload: string) => /^\b((mailto|tel|sms):|[a-z]+:\/\/)/i.test(payload);

export const BotMessage = forwardRef<HTMLDivElement | null, MessagePropsWithTopIntentName>(
    ...
    return (
      ...
      >
        ...
        <Box p="4" _empty={{ display: "none" }}>
          ...
          {text && custom ? <Separator orientation="horizontal" css={dividerSx} /> : null}

          {custom ? (
            <Box>
              <Flex alignItems="center">
                <Icon icon={IconCode} style={{ marginRight: getToken("spacing.7") }} />
                JSON
              </Flex>
              <Modal
                button={() => (
                  <Button size="sm" mt="3" colorPalette="gray" onClick={(e) => e.stopPropagation()}>
                    Click to view code
                  </Button>
                )}
...
const getButtons = (responseData?: ResponseData | null): PayloadItem[] => {
  ...
  const responseButtons = [...responseData.buttons, ...responseData.quickReplies].filter(Boolean);
