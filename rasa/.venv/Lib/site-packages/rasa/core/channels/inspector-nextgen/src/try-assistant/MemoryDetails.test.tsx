...
describe("SlotsDetails", () => {
  ...
  describe("System section", () => {
    it("renders for system slots from events", () => {
      // formatSlots is called 3 times: currentFlowSlots, sessionSlots, eventBasedSystemSlots
      vi.mocked(formatSlots)
        .mockReturnValueOnce([]) // currentFlowSlots
        .mockReturnValueOnce([]) // sessionSlots
        .mockReturnValueOnce([makeSlot("language", "en", makeSlotEvent("language"))]); // eventBasedSystemSlots
      vi.mocked(isSystemSlotEvent).mockImplementation((name) => name === "language");
    ...
    it("renders system slots from tracker that are missing from events", () => {
      vi.mocked(formatSlots).mockReturnValue([]);
      vi.mocked(isSystemSlotEvent).mockImplementation((name) => name === "language");

      renderWithProviders(<SlotsDetails />, {
        initialStoreState: {
          slotRelatedEvents: [],
          stack: [],
          slots: { language: "en" },
    ...
    it("does not duplicate system slots already covered by events", () => {
      const langEvent = makeSlotEvent("language");
      vi.mocked(formatSlots)
        .mockReturnValueOnce([]) // currentFlowSlots
        .mockReturnValueOnce([]) // sessionSlots
        .mockReturnValueOnce([makeSlot("language", "en", langEvent)]); // eventBasedSystemSlots
      vi.mocked(isSystemSlotEvent).mockImplementation((name) => name === "language");
      ...
      expect(screen.getByTestId("System-section")).toBeInTheDocument();
      const langSlots = screen.getAllByText("language");
