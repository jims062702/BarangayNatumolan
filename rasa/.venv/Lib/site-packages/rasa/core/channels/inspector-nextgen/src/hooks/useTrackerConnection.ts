...
import { getSlotRelatedEvents, isUtterance, mapRawEventsToConversationEvents } from "../utils";
...
export function useTrackerConnection({ enabled, channel }: { enabled: boolean; channel: string }) {
  ...
  useEffect(() => {
    ...
    const wsUrl = projectUrl.replace(/^http/, "ws") + `/webhooks/${channel}/tracker_stream`;
    ...
    ws.onmessage = (messageEvent) => {
      ...
      const conversation: Conversation = {
        ...
        reviewed: false,
        totalNumberOfUserMessages: events.filter((e: UnionEventType) => isUtterance(e)).length,
      };
