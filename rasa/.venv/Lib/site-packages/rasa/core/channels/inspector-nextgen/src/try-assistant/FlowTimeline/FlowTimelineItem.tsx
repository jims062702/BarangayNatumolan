...
export function FlowTimelineItem({ entry, isFirst, isLast, onClick }: FlowTimelineItemProps) {
  const { palette, label } = STATUS_CONFIG[entry.status];

  const title = useMemo(() => {
    if (entry.type === "agent") {
      return `Sub-agent: ${entry.agentId ?? "unknown"}`;
  ...
  return (
    ...
    >
      ...
      <Flex direction="column" alignItems="center" width="6" flexShrink={0}>
        {isFirst ? (
          <Box height="4" flexShrink={0} />
        ) : (
          <Box width="px" height="4" bg="border.emphasized" flexShrink={0} />
        )}
        <Box width="2" height="2" borderRadius="full" bg="colorPalette.solid" flexShrink={0} />
        {isLast ? <Box flex={1} /> : <Box width="px" flex={1} bg="border.emphasized" />}
      </Flex>
