...
function renderUserMessageInfo(utterance: Utterance, onClose: () => void = () => undefined) {
  return renderWithProviders(<UserMessageInfo utterance={utterance} onClose={onClose} />);
}

describe("UserMessageInfo", () => {
  it("uses 'User message details' as the panel title", () => {
    renderUserMessageInfo(baseUtterance());
    expect(screen.getByRole("heading", { name: /user message details/i })).toBeInTheDocument();
  });
  ...
  describe("Predicted intents", () => {
    it("shows a dash when intents are absent", () => {
      ...
      expect(screen.getByRole("heading", { name: /predicted intents/i })).toBeInTheDocument();
  ...
  describe("Predicted Commands", () => {
    ...
    it("renders the commands section when commands is present", () => {
      renderUserMessageInfo(
        baseUtterance({
          commands: [{ type: "SetSlot", name: "city", value: "Berlin" }],
        }),
      );
      expect(screen.getByRole("heading", { name: /predicted commands/i })).toBeInTheDocument();
    });
    ...
    it("renders commands as formatted JSON", () => {
      ...
      expect(
        screen.getByText(
          (_, el) => el?.tagName === "CODE" && (el.textContent ?? "").includes('"type": "SetSlot"'),
        ),
      ).toBeInTheDocument();
    });
  });
