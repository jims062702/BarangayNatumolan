...
describe("Icon (canvas node)", () => {
  describe("renders without error for each node type", () => {
    ...
    it("Call (Flow)", () => {
      const { container } = renderWithProviders(<Icon node={callNode(CallType.Flow)} />);
      expect(getSvgIcon(container)).toBeInTheDocument();
    });

    it("Call (Agent)", () => {
      const { container } = renderWithProviders(<Icon node={callNode(CallType.Agent)} />);
      expect(getSvgIcon(container)).toBeInTheDocument();
    });

    it("Call (Tool)", () => {
      const { container } = renderWithProviders(<Icon node={callNode(CallType.Tool)} />);
      expect(getSvgIcon(container)).toBeInTheDocument();
    });
  });
  ...
  describe("Call node icons differ by callType", () => {
    it("Flow uses arrow-right-arrow-left icon", () => {
      const { container } = renderWithProviders(<Icon node={callNode(CallType.Flow)} />);
      expect(
        container.querySelector('svg[data-icon="arrow-right-arrow-left"]'),
      ).toBeInTheDocument();
    });
