...
export const TryAssistantConversation = ({
  ...
  const lastEventId = lastConversation?.events[lastConversation.events.length - 1]?.id;
  ...
  const nonEmptyConversationList = conversationList.filter(
    (conversation, index) =>
      conversation.totalNumberOfUserMessages > 0 || index === conversationList.length - 1,
  );
      ...
      {nonEmptyConversationList.map((conversation, index) => (
        <ConversationSession
          ...
          waitingForResponse={
            ...
            waitingForResponse &&
            nonEmptyConversationList[index].totalNumberOfUserMessages > 0
          }
          replayingConversation={
            index === nonEmptyConversationList.length - 1 && replayingConversation
          }
