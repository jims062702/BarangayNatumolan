...
export const ChatSection = ({ handleSelect }: ChatSectionProps) => {
  ...
  const replayingConversation = useInspectorStore((s) => s.replayingConversation);
  const waitingForResponse = useInspectorStore(selectWaitingForResponse);
  const conversationEventActions = useInspectorStore((s) => s.conversationEventActions);
  const sendMessage = useInspectorStore((s) => s.sendMessage);
  const startNewConversation = useInspectorStore((s) => s.startNewConversation);
  const replayConversationAction = useInspectorStore((s) => s.replayConversation);
  ...
  const setFlowView = useCallback((value: boolean) => setInspectMode(value), []);
