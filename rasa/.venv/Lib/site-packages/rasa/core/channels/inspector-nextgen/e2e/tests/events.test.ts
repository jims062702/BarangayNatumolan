...
test.describe("Inspector events", () => {
  ...
  test("Flow events are visible in inspect mode", async ({ inspectorPage }) => {
    await test.step("Assert flow canvas with nodes is visible", async () => {
    ...
    await test.step("Assert the initial finance demo flow state", async () => {
      await ui.inspectorCanvas.assertions(inspectorPage).activeFlowNameIsVisible("say hello");

      for (const nodeName of greetingFlowNodes) {
        await ui.inspectorCanvas.assertions(inspectorPage).flowNodeIsVisible(nodeName);
      }

      for (const eventName of initialConversationEvents) {
        await ui.conversationLog.assertions(inspectorPage).conversationEventIsVisible(eventName);
      }
    });
