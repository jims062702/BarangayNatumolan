...
import { SquareCheck } from "../../../Icon/icons";
import {
...
function generateSlotEventMessage(event: ConversationEvent) {
  if (event.conversationEventType === ConversationEventType.ResetSlots) {
    return <Text textStyle="xs">Slots reset</Text>;
  }

  const isSlotCleared =
    (event.conversationEventType === ConversationEventType.Slot && event.metadata?.reset) ||
    event.slotValue === null;

  const suffix = isSlotCleared ? "is cleared" : "set";

  const modifiedSuffix = event.metadata?.was_modified_by_studio ? " (modified)" : "";
    ...
    return (
      ...
      >
        ...
        <Box css={baseMessageSx}>
          <Icon icon={SquareCheck} style={iconSx} />
