...
export const assertions = (page: Page) => {
  ...
  return {
    voiceStartButtonIsVisible: async () => {
      await expect(locators.voiceStartButton, "Voice start button should be visible").toBeVisible();
    },
    voiceStartButtonIsHidden: async () => {
      await expect(locators.voiceStartButton, "Voice start button should be hidden").toBeHidden();
    },
    voiceStopButtonIsVisible: async () => {
      await expect(locators.voiceStopButton, "Voice stop button should be visible").toBeVisible();
    },
    ...
    sendMessageButtonIsHidden: async () => {
      await expect(locators.sendMessageButton, "Send message button should be hidden").toBeHidden();
    },
    ...
    activeStateIsVisible: async () => {
      ...
      ).toHaveAttribute("placeholder", /Voice conversation in progress \(\d{2}:\d{2}\)/);
      await expect(
        locators.inputField,
    ...
    timerHasIncremented: async () => {
      const initialPlaceholder = await locators.inputField.getAttribute("placeholder");
      ...
      await expect
        .poll(
          ...
          {
            message: `Voice timer did not increment or voice call ended unexpectedly. Initial placeholder: "${initialPlaceholder}"`,
            timeout: 10000,
          },
        )
        .not.toBeNull();
