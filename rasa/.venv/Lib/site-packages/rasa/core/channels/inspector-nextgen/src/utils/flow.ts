...
import { DEFAULT_NODE_WIDTH, DEFAULT_NODE_MAX_HEIGHT } from "../constants";
...
export const nodePropsToNodeObject = (nodeProps: NodeProps<CanvasNodeData>): Node => {
...
export async function autoLayout(nodes: FlowNode[], allEdges: FlowEdge[]): Promise<FlowNode[]> {
  const elkNodes: ElkNode[] = nodes.map((flowNode) => ({
    id: flowNode.id,
    width: DEFAULT_NODE_WIDTH,
    height: DEFAULT_NODE_MAX_HEIGHT,
  }));
  ...
  const meaningfulEdges = allEdges.filter((edge) => edge.type === FlowEdgeType.Auto);
  ...
  const orderedEdges = meaningfulEdges.sort((edgeA, edgeB) => {
    ...
    if (nodeA.type === FlowNodeType.Condition && nodeA.subType === NodeConditionSubType.ELSE) {
      return 1;
    }

    if (nodeB.type === FlowNodeType.Condition && nodeB.subType === NodeConditionSubType.ELSE) {
      return -1;
    }

    if (nodeA.type === FlowNodeType.Condition && nodeB.type === FlowNodeType.Condition) {
