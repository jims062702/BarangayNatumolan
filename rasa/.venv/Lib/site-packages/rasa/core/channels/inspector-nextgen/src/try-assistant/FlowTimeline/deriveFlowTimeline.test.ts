...
describe("deriveFlowTimeline", () => {
  ...
  it("resolves flow names from provided map", () => {
    ...
    const flowNames = new Map([["payment_processing", "Payment Processing"]]);
  ...
  it("orders agent entry before flow entry when both share a flowId and agent started sub-millisecond later", () => {
    ...
    const events: UnionEventType[] = [
      makeFlowEvent({
        ...
        originalTimestamp: 1704110400.0,
      }),
      makeFlowEvent({
        conversationEventType: ConversationEventType.AgentStarted,
        flowId: "my_flow",
  ...
  it("marks flows as interrupted when referenced in pattern_continue_interrupted metadata", () => {
    const events: UnionEventType[] = [
      makeFlowEvent({
        conversationEventType: ConversationEventType.FlowStarted,
      ...
      makeFlowEvent({
        ...
        originalTimestamp: 1704110461,
      }),
      makeFlowEvent({
        conversationEventType: ConversationEventType.FlowStarted,
        flowId: "pattern_continue_interrupted",
        timestamp: "2024-01-01T12:02:00Z",
        originalTimestamp: 1704110520,
        metadata: {
          interrupted_flow_ids: ["transfer_money", "add_contact"],
        } as EventMetadata,
      }),
    ];
    ...
    const transferEntry = result.find((e) => e.flowId === "transfer_money");
    const addContactEntry = result.find((e) => e.flowId === "add_contact");
    const patternEntry = result.find((e) => e.flowId === "pattern_continue_interrupted");
    expect(transferEntry?.status).toBe("interrupted");
    expect(addContactEntry?.status).toBe("interrupted");
    expect(patternEntry?.status).toBe("active");
  });

  it("does not duplicate interrupted status for flows that already have flow_interrupted event", () => {
    const events: UnionEventType[] = [
      makeFlowEvent({
        id: "start-1",
        conversationEventType: ConversationEventType.FlowStarted,
      ...
      makeFlowEvent({
        ...
        originalTimestamp: 1704110520,
        metadata: {
          interrupted_flow_ids: ["transfer_money"],
        } as EventMetadata,
      }),
    ];
    ...
    expect(result).toHaveLength(2);
    const transferEntry = result.find((e) => e.flowId === "transfer_money");
    expect(transferEntry?.flowId).toBe("transfer_money");
    expect(transferEntry?.status).toBe("interrupted");
    expect(transferEntry?.id).toBe("start-1");
    expect(result.find((e) => e.flowId === "pattern_continue_interrupted")?.status).toBe("active");
  });

  it("keeps pattern_continue_interrupted entry even when referenced flows do not exist", () => {
    const events: UnionEventType[] = [
      makeFlowEvent({
        id: "pattern-start",
        conversationEventType: ConversationEventType.FlowStarted,
        ...
        originalTimestamp: 1704110520,
        metadata: {
          interrupted_flow_ids: ["nonexistent_flow"],
        } as EventMetadata,
      }),
    ];
    ...
    expect(result).toHaveLength(1);
    expect(result[0].flowId).toBe("pattern_continue_interrupted");
    expect(result[0].status).toBe("active");
    expect(result[0].id).toBe("pattern-start");
  });
});
