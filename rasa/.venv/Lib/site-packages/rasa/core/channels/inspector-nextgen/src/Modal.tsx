              ...
              <Box as={type} css={overflowSx}>
                ...
                {canBeClosed && (
                  <Dialog.CloseTrigger asChild>
                    <IconButton size="lg" variant="subtle" colorPalette="gray" aria-label="Close">
                      <Icon icon={XMark} />
                    </IconButton>
                  </Dialog.CloseTrigger>
                )}
...
const Actions = ({ actions }: { actions: Action[] }) => {
  return (
    <HStack gap="2" flexGrow={1} justifyContent="flex-end">
      {actions.map((action) => {
        const { id, title, onClick, type = "button", linksTo, ...props } = action;
        ...
        return (
          <Button key={id} type={type} onClick={handleClick} borderRadius="lg" {...props}>
            {title}
          </Button>
        );
      })}
