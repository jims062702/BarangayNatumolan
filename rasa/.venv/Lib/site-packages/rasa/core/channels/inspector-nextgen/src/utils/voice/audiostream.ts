...
export const createAudioQueue = (socket: Socket): AudioQueue => {
  ...
  const pushToWorklet = (audio: Float32Array) => {
    playbackNode!.port.postMessage({ type: "audio", data: audio }, [audio.buffer]);
  };
  ...
  const queue: AudioQueue = {
    ...
    onSamplesPlayed(samplesPlayed: number) {
      ...
      this.popMarkers();
      if (this.queuedSamples === 0 && drainResolvers.length > 0) {
    ...
    reduceMarkers(samplesPlayed: number) {
      this.marks = this.marks.map((marker_input) => ({
        ...marker_input,
        bytesToGo: marker_input.bytesToGo - samplesPlayed,
      }));
    },
    ...
    waitForDrain(timeoutMs = 10000): Promise<void> {
      if (this.queuedSamples === 0) return Promise.resolve();
      return new Promise<void>((resolve) => {
        ...
        drainResolvers.push(settle);
        setTimeout(() => {
          const idx = drainResolvers.indexOf(settle);
          if (idx !== -1) drainResolvers.splice(idx, 1);
          settle();
        }, timeoutMs);
      });
    },
...
export const streamMicrophoneToServer = async (
  ...
  try {
    ...
    const microphoneNode = new AudioWorkletNode(audioContext, "microphone-processor");
    ...
    microphoneNode.port.onmessage = (event: MessageEvent) => {
      if (event.data instanceof Float32Array) {
        ...
        logError(`Received unexpected data type from microphone-processor`, {
          ...
          extra: {
            data: typeof event.data === "object" ? JSON.stringify(event.data) : null,
          },
        });
      }
    };
...
export const stopMicrophoneStream = async (
  ...
  const { audioStream, source, microphoneNode, audioContext } = microphoneStream;
...
export const setupAudioPlayback = async (
  ...
  globalPlaybackNode = new AudioWorkletNode(globalAudioOutputContext, "playback-processor");

  audioQueue.attachPlaybackNode(globalPlaybackNode);

  globalPlaybackNode.port.onmessage = (event: MessageEvent<{ type: string; samples: number }>) => {
...
export const stopAudioPlayback = async (audioQueue: AudioQueue | undefined): Promise<void> => {
...
export const addDataToAudioQueue = (audioQueue: AudioQueue) => (message: string) => {
  ...
  } else if (parsedData["marker"] && typeof parsedData["marker"] === "string") {
