...
describe("downloadRawTrackerLogs", () => {
  ...
  it("triggers download with JSON content", () => {
    ...
    downloadRawTrackerLogs(conversations, "sess-1");
...
describe("formatConversationForCopy", () => {
  it("formats user and bot messages as a plain transcript", () => {
    const conversation = makeConversation([
      makeUtterance({ type: UtteranceType.User, text: "Hello" }),
      makeUtterance({ type: UtteranceType.Bot, text: "Hi there!" }),
    ]);

    const result = formatConversationForCopy(conversation);

    expect(result).toBe("User: Hello\nAgent: Hi there!");
  });

  it("skips internal user messages like /session_start", () => {
    const conversation = makeConversation([
      makeUtterance({ type: UtteranceType.User, text: "/session_start" }),
      makeUtterance({ type: UtteranceType.User, text: "Hey" }),
      makeUtterance({ type: UtteranceType.Bot, text: "Hello!" }),
    ]);
    ...
    expect(result).not.toContain("/session_start");
    expect(result).toBe("User: Hey\nAgent: Hello!");
  });

  it("skips conversation events and only includes messages", () => {
    const conversation = makeConversation([
      makeUtterance({ type: UtteranceType.User, text: "Transfer 50" }),
      makeConversationEvent({
        conversationEventType: ConversationEventType.FlowStarted,
      ...
      makeUtterance({ type: UtteranceType.Bot, text: "To whom?" }),
    ]);

    const result = formatConversationForCopy(conversation);

    expect(result).toBe("User: Transfer 50\nAgent: To whom?");
  });
  ...
  it("handles multi-turn conversations", () => {
    const conversation = makeConversation([
      ...
      makeUtterance({
        type: UtteranceType.Bot,
        text: "Who would you like to transfer to?",
      }),
    ]);
    ...
    expect(result).toBe(
      [
        ...
        "Agent: Hi!",
        "User: Money transfer",
        "Agent: Who would you like to transfer to?",
      ].join("\n"),
    );
  });
});
...
describe("copyConversation", () => {
  ...
  it("copies only the last conversation", async () => {
    const firstConv = makeConversation([
      makeUtterance({ type: UtteranceType.User, text: "Old message" }),
    ]);
    const lastConv = makeConversation([
      makeUtterance({ type: UtteranceType.User, text: "New message" }),
    ]);

    await copyConversation([firstConv, lastConv]);

    expect(writeTextMock).toHaveBeenCalledOnce();
    const clipboard = writeTextMock.mock.calls[0][0] as string;
    expect(clipboard).toContain("User: New message");
    expect(clipboard).not.toContain("Old message");
  });

  it("does not write to clipboard when conversations list is empty", async () => {
