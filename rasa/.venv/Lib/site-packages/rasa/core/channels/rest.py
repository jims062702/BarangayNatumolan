...
class _RestOutputChannel(CollectingOutputChannel):
    """Output channel for the built-in, non-streaming REST webhook handler.

    Internal use only - unlike `CollectingOutputChannel`, which is a documented
    public API for custom connectors to construct directly and read
    `tracker_state` from (see the custom-connectors docs), this subclass exists
    solely for Rasa's own `/webhooks/rest/webhook` handling below, which never
    reads `tracker_state`. Safe to skip that computation here without touching
    the public `CollectingOutputChannel` contract.
    """

    @property
    def wants_tracker_state(self) -> bool:
        """This channel never reads tracker_state."""
        return False
