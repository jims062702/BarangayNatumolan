...
class SocketIOOutput(OutputChannel):
    ...
    def supports_streaming(self) -> bool:
        """Indicates that the channel supports streaming responses."""
        return True
    ...
    async def send_response_chunk_start(
        ...
        await self.sio_server.emit("stream_start", {}, room=recipient_id)

    async def send_response_chunk(
        self, recipient_id: Text, chunk: Text, **kwargs: Any
    ) -> None:
        """Send a chunk of response to the client via Socket.IO."""
    ...
    async def send_response_chunk_end(self, recipient_id: Text, **kwargs: Any) -> None:
        ...
        await self.sio_server.emit("stream_end", {}, room=recipient_id)

    async def _send_message(self, socket_id: Text, response: Any) -> None:
        """Sends a message to the recipient using the bot event."""
        logger.debug(
            "socketio_channel._send_message",
            socket_id=socket_id,
...
class SocketIOInput(InputChannel):
    ...
    def get_output_channel_for_conversation(
        ...
        if conversation_id not in self._sid_to_sender.values():
            return None
    ...
    async def _monitor_silence_timeout(self, silence_timeout: SilenceTimeout) -> None:
        ...
        # this is important if the monitoring task is cancelled while handler is
        # executed
        asyncio.get_event_loop().create_task(
