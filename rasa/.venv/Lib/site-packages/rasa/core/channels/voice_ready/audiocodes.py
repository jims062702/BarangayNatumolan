...
class AudiocodesInput(InputChannel):
    ...
    def _get_conversation_securely(
        self, token: Optional[Text], conversation_id: Text
    ) -> Conversation:
        self._check_token(token)
        return self.get_conversation(conversation_id)

    def get_conversation(self, conversation_id: Text) -> Conversation:
    ...
    def blueprint(
        ...
        async def new_client_connection(
            ...
            conversation = self._get_conversation_securely(
                request.token, conversation_id
            )
            if conversation:
                if conversation.ws:
                    structlogger.debug(
        ...
        async def on_activities(
            ...
            ac_output = self.create_output_channel(conversation_id, request.token)
        ...
        async def keepalive(request: Request, conversation_id: Text) -> HTTPResponse:
            ...
            """
            self._get_conversation_securely(request.token, conversation_id)
            return response.json({})
    ...
    def create_output_channel(
        self, conversation_id: str, token: Optional[str] = None
    ) -> Union[WebsocketOutput, AudiocodesOutput]:
        if self.use_websocket:
            # send an empty response for this request
            # activities are processed in the background
            # chat response is sent via the websocket
            ...
            if token:
                conversation = self._get_conversation_securely(token, conversation_id)
            else:
