...
class DTMFInputEvent(VoiceInputEvent):
    ...
    Attributes:
        ...
        slot_name: The slot to fill directly via SetSlotCommand, bypassing CommandGen.
            When set, the generated UserMessage carries pre-populated parse_data so
            the processor skips the LLM round-trip entirely (VO-407).
    """
    ...
    slot_name: Optional[str] = field(default=None, compare=False)
    ...
    def to_user_message(
        ...
        """Build UserMessage with SetSlotCommand injected when slot_name is set."""
        if self.slot_name is None:
            return super().to_user_message(output_channel, sender_id, input_channel)

        # Lazy import to avoid circular import via rasa.core.channels.__init__
        ...
        command = SetSlotCommand(
            name=self.slot_name,
            value=self.text,
            extractor=SetSlotExtractor.CUSTOM.value,
        )
        parse_data = {
