...
class CallState:
    ...
    tts_read_from_cache: bool = False

    # DTMF State
    is_collecting_dtmf: bool = False
    dtmf_config: Optional[DTMFConfig] = None
    dtmf_buffer: str = ""
    dtmf_slot: Optional[str] = None

    current_bot_utterance_type: Optional[StepType] = None
    audio_markers: Dict[str, Marker] = field(default_factory=dict)

    # True when the client acks playback markers, so silence monitoring can wait for
    # real playback to complete. Channels that never ack must keep this False.
    tracks_audio_playback: bool = True
    ...
    def has_pending_bot_audio(self) -> bool:
        """Check whether bot audio has been sent but has not finished playing.

        Playback markers are added when audio is streamed to the client and removed
        once the client acknowledges reaching them. While any marker is still
        outstanding the client is either yet to start or still playing the bot's audio,
        so the user has not actually been given the floor yet and we must not treat
        their (expected) silence as a timeout.
        """
        return self.tracks_audio_playback and bool(self.audio_markers)
    ...
    def start_silence_monitoring(self) -> None:
        # Cancel any in-flight watcher before (re)starting so we never leak tasks.
        self.stop_silence_monitoring()
        logger.debug(
            "voice_channel.start_silence_monitoring.create_task",
            silence_timeout=self.silence_timeout,
            is_rasa_listening=self.is_rasa_listening,
            is_bot_speaking=self.is_bot_speaking,
            is_user_speaking=self.is_user_speaking,
            has_pending_bot_audio=self.has_pending_bot_audio(),
        )
    ...
    def stop_silence_monitoring(self) -> None:
        if self.silence_timeout_watcher:
            logger.debug(
                "voice_channel.stop_silence_monitoring",
                is_rasa_listening=self.is_rasa_listening,
                is_bot_speaking=self.is_bot_speaking,
                is_user_speaking=self.is_user_speaking,
                has_pending_bot_audio=self.has_pending_bot_audio(),
            ...
            while True:
                try:
                    ...
                    if (
                        ...
                        and not self.has_pending_bot_audio()
                    ):
