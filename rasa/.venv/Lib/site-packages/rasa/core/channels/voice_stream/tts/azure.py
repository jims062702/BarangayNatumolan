...
class _AudioOutputCallback(speechsdk.audio.PushAudioOutputStreamCallback):
    """Bridge Azure Speech SDK audio output to an asyncio queue.

    Azure Speech SDK uses a separate thread to handle audio streaming. So
    in order to push audio bytes back to the async loop in which Azure TTS
    is running we need to encapsulate that loop and queue into one object
...
class AzureTTS(TTSEngine[AzureTTSConfig]):
    ...
    async def set_language(self, rasa_language: str) -> bool:
        ...
        if not await super().set_language(rasa_language):
            return False
        # Recycle the underlying SDK synthesizer under the engine lock so a
        # concurrent ``send_text_chunk`` can't observe it being torn down.
