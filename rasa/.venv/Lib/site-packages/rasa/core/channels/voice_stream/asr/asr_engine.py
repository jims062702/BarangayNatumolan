...
class ASRLanguageMapEntry(BaseModel):
    ...
    Attributes:
        language: ASR-specific language code (e.g., 'en-US' for Azure).
            Optional for engines that do not use a language query parameter
            (e.g. Deepgram v2/Flux, which infers language from the model).
...
class ASREngine(Generic[T]):
    ...
    def __init__(
        ...
        self.config = self._resolve_config(config, rasa_language)
        ...
        # Serializes send_audio_chunks against the close+reopen in set_language.
        # Lazy so it binds to the running event loop on first use.
    ...
    def _resolve_config(self, config: Optional[T], rasa_language: str) -> T:
        """Build the final config by merging caller values onto the defaults."""
        merged = self.get_default_config(rasa_language).merge(config)
        return cast(T, merged.apply_deprecated_fields(rasa_language))
    ...
    async def close_connection(self) -> None:
        if self.asr_socket:
            ...
            # Null out so concurrent producers see "not connected" instead of
            # sending on a closed socket and crashing the call task.
    ...
    async def send_audio_chunks(self, chunk: RasaAudioBytes) -> None:
        ...
        If the engine is currently being reconnected (e.g. due to a mid-call
        language switch) or the socket has been closed by the remote end, the
        chunk is dropped silently rather than tearing down the call task.
        """
        async with self._get_engine_lock():
            socket = self.asr_socket
            if socket is None or socket.closed:
                logger.debug(
                    f"asr.{self.name()}.send_audio_chunks.skipped",
                    reason=(
                        "socket_not_connected" if socket is None else "socket_closed"
                    ),
                )
                return
            engine_bytes = self.rasa_audio_bytes_to_engine_bytes(chunk)
            try:
                await socket.send(engine_bytes)
            except (
                websockets.exceptions.ConnectionClosedOK,
                websockets.exceptions.ConnectionClosedError,
            ) as e:
                # Closed concurrently by set_language or the remote engine —
                # drop this chunk; subsequent chunks use the new socket.
                logger.debug(
                    f"asr.{self.name()}.send_audio_chunks.connection_closed",
                    error=str(e),
                )
    ...
    async def set_language(self, rasa_language: str) -> None:
        ...
        # Hold the engine lock across close+open so concurrent send_audio_chunks
        # callers either run before close or after the new connect.
        async with self._get_engine_lock():
