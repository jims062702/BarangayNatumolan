...
class DeepgramTTS(TTSEngine[DeepgramTTSConfig]):
    ...
    async def connect(self, config: Optional[DeepgramTTSConfig] = None) -> None:
        ...
        self.ws = await self.session.ws_connect(
            ws_url,
            headers=headers,
            timeout=ClientWSTimeout(ws_close=float(self.config.timeout)),
        )
    ...
    async def set_language(self, rasa_language: str) -> bool:
        ...
        if not await super().set_language(rasa_language):
            return False
        # Recycle the websocket under the engine lock so concurrent writers
        # (send_text_chunk, signal_text_done) can't observe a half-closed ws.
