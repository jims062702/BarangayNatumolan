...
class InspectorTrackerUpdatePlugin:
    ...
    @hookimpl
    def after_prediction_loop_end(self, tracker: "DialogueStateTracker") -> None:
        """Triggers a tracker update after the prediction loop ends.

        Ensures the inspector receives the final tracker state, including the
        ``BotTurnEnded`` event with turn-level execution-time metadata.
        """
        self.handle_tracker_update(tracker)
...
class InspectorInputChannel(SocketIOInput, VoiceInputChannel):
    ...
    async def on_message_proxy(
        ...
        Surfaces an agent-not-ready error to the inspector UI.
        """
        try:
    ...
    async def handle_session_request(
        ...
        # If a tracker already exists for this session, push it immediately
        # so the inspector can display a previously-saved conversation.
        if self.agent is not None:
            try:
                existing_tracker = await self.agent.tracker_store.retrieve(
                    data["session_id"]
                )
                if existing_tracker is not None and len(existing_tracker.events) > 0:
                    tracker_dump = tracker_as_dump(existing_tracker)
                    # Push to room=sid (the connecting socket) not room=sender_id.
                    # Live updates go to room=sender_id via publish_tracker_update;
                    # with session_persistence=True the client enters that room on
                    # connect so both the initial dump and live events are received.
                    # If session_persistence is disabled the client won't join the
                    # sender_id room and will miss subsequent live tracker events.
                    await self.sio_server.emit(  # type: ignore[union-attr]
                        "tracker", tracker_dump, room=sid
                    )
                    structlogger.debug(
                        "inspector.handle_session_request.pushed_existing_tracker",
                        session_id=data["session_id"],
                    )
            except Exception as exc:
                structlogger.warning(
                    "inspector.handle_session_request.tracker_lookup_failed",
                    session_id=data["session_id"],
                    error=str(exc),
                )
    ...
    def _start_voice_session(
        ...
        if sid in self.active_connections:
            structlogger.warning(
                "inspector._start_voice_session.restarting_existing_session",
                session_id=sid,
            )
