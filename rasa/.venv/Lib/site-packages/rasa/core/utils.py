...
def cancel_flow_and_push_internal_error(stack: DialogueStack, flow_name: str) -> None:
    ...
    # Only cancel when there is actually a user flow to cancel. The residual
    # stack from a previous session can hold pattern frames alone (e.g. when
    # action_session_start fails before clearing the stack) — in that case
    # there is nothing to cancel and we only push the internal error pattern.
    if top_user_flow_frame(stack) is not None:
