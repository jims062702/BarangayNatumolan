...
class AWSElasticacheRedisIAMCredentialsProvider(redis.CredentialProvider):
    ...
    # AWS ElastiCache presigned URLs are valid for 15 minutes
    TOKEN_LIFETIME_SECONDS = 900
    # Refresh the token this many seconds before it expires to avoid handing a
    # near-expired URL to redis-py on a reconnect.
    ...
    def __init__(self, username: str, cluster_name: Optional[str] = None) -> None:
        ...
        # redis-py's sync ConnectionPool can call get_credentials() from multiple
        # threads concurrently; serialize re-signs to avoid duplicate work.
    ...
    def get_credentials(self) -> Tuple[str, str]:
        ...
        Used internally by redis-py when (re)connecting to Redis. Refreshes the
        presigned URL when within `TOKEN_REFRESH_MARGIN_SECONDS` of expiry so a stale
        URL is never handed to redis-py — see PF-1037 (token not refreshed after
        >30 min of idle pro-server time).
        """
        with self._sign_lock:
            now = time.time()
            if (
                self._credentials is None
                or now >= self._expires_at - self.TOKEN_REFRESH_MARGIN_SECONDS
            ):
