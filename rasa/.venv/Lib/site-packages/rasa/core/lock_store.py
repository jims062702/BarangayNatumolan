...
from rasa.exceptions import ValidationError as RasaValidationError
...
_INVALID_TICKET_LOCK_LIFETIME_CODE = (
    "lock_store.get_lock_lifetime.invalid_ticket_lock_lifetime"
)


def get_lock_lifetime() -> int:
    """Return conversation ticket lock lifetime in seconds from the environment."""
    raw_value = os.environ.get("TICKET_LOCK_LIFETIME")
    if raw_value is None or raw_value == "":
        return DEFAULT_LOCK_LIFETIME

    try:
        lock_lifetime = int(raw_value)
        if lock_lifetime < 0:
            raise ValueError
    except ValueError as error:
        raise RasaValidationError(
            code=_INVALID_TICKET_LOCK_LIFETIME_CODE,
            event_info=(
                "TICKET_LOCK_LIFETIME must be a non-negative integer (seconds), "
                f"but got '{raw_value}'."
            ),
        ) from error

    return lock_lifetime or DEFAULT_LOCK_LIFETIME
...
class LockStore:
    ...
    def issue_ticket(
        ...
        """Issue new ticket with `lock_lifetime` for `conversation_id`.
