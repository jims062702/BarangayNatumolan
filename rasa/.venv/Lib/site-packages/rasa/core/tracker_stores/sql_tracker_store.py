...
# Maximum number of session-boundary fetches attempted (newest first) before
# falling back to a single full-history fetch.
_MAX_SESSION_BOUNDARY_ATTEMPTS = 3
...
class SQLTrackerStore(TrackerStore, SerializedTrackerAsText):
    ...
    async def retrieve(self, sender_id: str) -> Optional[DialogueStateTracker]:
        ...
        Fetches the minimum event prefix needed for a replay-safe slice using
        progressive session-boundary widening.  The boundary type is chosen
        from the domain session config (``action_session_start`` when
        ``start_session_after_expiry`` is ``True``, ``ConversationInactive``
        otherwise).  Falls back to a single full-history fetch only when all
        session boundaries are exhausted.
        """
        with self.session_scope() as session:
            return self._replay_safe_tracker_from_db(session, sender_id)
    ...
    def _event_query_from_id(
        self, session: "Session", sender_id: str, min_event_id: Optional[int]
    ) -> "Query":
        """Return an ordered event query filtered by a minimum event ``id``.

        Filtering by integer primary key is more reliable than timestamp-based
        filtering because IDs are strictly monotonic and always unique, whereas
        ``timestamp`` is a low-precision float that can collide.

        Args:
            session: Current database session.
            sender_id: Conversation ID to fetch events for.
            min_event_id: Inclusive lower bound on ``id``. ``None`` returns
                all events (full-history fetch).

        Returns:
            Query yielding :class:`SQLEvent` rows ordered by ``id``.
        """
        query = session.query(self.SQLEvent).filter(
            self.SQLEvent.sender_id == sender_id
        )
        if min_event_id is not None:
            query = query.filter(self.SQLEvent.id >= min_event_id)
        return query.order_by(self.SQLEvent.id)

    def _session_boundary_event_ids(
        self, session: "Session", sender_id: str
    ) -> List[int]:
        """Return event ``id``s of session boundaries, newest first.

        When ``start_session_after_expiry`` is ``True``, returns the ``id``s of
        ``ActionExecuted(action_session_start)`` events.  When ``False``, returns
        the ``id``s of ``ConversationInactive`` events.

        The caller converts each boundary event ``id`` to a fetch lower bound via
        :meth:`_min_fetch_event_id_for_session_boundary` — inclusive for
        ``action_session_start``, exclusive for ``ConversationInactive`` (see
        ...
        Returns:
            Up to :data:`_MAX_SESSION_BOUNDARY_ATTEMPTS` event ``id``s, ordered
        ...
        if start_session_after_expiry:
            rows = (
                session.query(self.SQLEvent.id)
                .filter(
                    self.SQLEvent.sender_id == sender_id,
                    self.SQLEvent.type_name == ActionExecuted.type_name,
                    self.SQLEvent.action_name == ACTION_SESSION_START_NAME,
                ...
                .limit(_MAX_SESSION_BOUNDARY_ATTEMPTS)
                .all()
            )
        else:
            rows = (
                ...
                .filter(
                    self.SQLEvent.sender_id == sender_id,
                    self.SQLEvent.type_name == ConversationInactive.type_name,
                )
                .order_by(self.SQLEvent.id.desc())
                .limit(_MAX_SESSION_BOUNDARY_ATTEMPTS)
                .all()
    ...
    def _min_fetch_event_id_for_session_boundary(self, boundary_event_id: int) -> int:
        """Return the minimum event ``id`` to fetch for a session boundary.

        * ``start_session_after_expiry=True`` — include the
          ``action_session_start`` event (``id >= boundary_event_id``).
        * ``start_session_after_expiry=False`` — exclude ``ConversationInactive``
          and start at the next event (``id > boundary_event_id``), matching
          ``inactive_indices[-1] + 1`` in
          :func:`~rasa.shared.core.trackers.get_latest_replay_safe_session_tracker`.

        Args:
            boundary_event_id: ``id`` of the boundary event row.

        Returns:
            Inclusive lower bound for :meth:`_event_query_from_id`.
        """
        if self.domain.session_config.start_session_after_expiry:
            return boundary_event_id
        return boundary_event_id + 1

    @staticmethod
    def _last_stack_marker_event_id_from_rows(rows: list) -> Optional[int]:
        """Return the last stack event whose patch applies to an empty stack.

        When full tracker reconstruction fails with a patch error, a stack event
        whose patch can be applied to
        :class:`~rasa.dialogue_understanding.stack.dialogue_stack.DialogueStack.empty`
        without error marks the start of a segment that does not depend on earlier
        stack state.

        Args:
            rows: :class:`SQLEvent` ORM rows (need not be stack-only).

        Returns:
            Event ``id`` of the last such marker, or ``None`` if none found.
        """
        from rasa.dialogue_understanding.stack.dialogue_stack import DialogueStack

        last_marker_id: Optional[int] = None
        for row in rows:
            if row.type_name != DialogueStackUpdated.type_name:
                continue
            try:
                event_dict = json.loads(row.data)
                patch = event_dict.get("update", "[]")
                DialogueStack.empty().update_from_patch(patch)
                last_marker_id = row.id
            except (
                JsonPatchException,
    ...
    def _try_build_tracker_from_rows(
        self,
        session: "Session",
        sender_id: str,
        rows: list,
    ) -> Optional[DialogueStateTracker]:
        """Build a tracker from ``rows`` if all events can be replayed.

        Returns ``None`` when reconstruction raises
        :class:`jsonpatch.JsonPatchException`,
        :class:`jsonpointer.JsonPointerException`, :class:`json.JSONDecodeError`,
        or :class:`TypeError` (corrupt row JSON or stack ``update`` payload).
        """
        try:
            return self._build_stored_tracker(session, sender_id, rows)
        except (
            JsonPatchException,
            JsonPointerException,
            json.JSONDecodeError,
    ...
    def _build_tracker_from_event_id(
        self,
        session: "Session",
        sender_id: str,
        min_event_id: int,
    ) -> Optional[DialogueStateTracker]:
        """Fetch from ``min_event_id`` and build a tracker if possible.

        1. Fetch ``id >= min_event_id`` and attempt tracker reconstruction.
        2. If build raises :class:`jsonpatch.JsonPatchException`,
           :class:`jsonpointer.JsonPointerException`, :class:`json.JSONDecodeError`,
           or :class:`TypeError`, scan stack events in the fetched rows for the
           last one whose patch applies to
           :class:`~rasa.dialogue_understanding.stack.dialogue_stack.DialogueStack.empty`
           without error, then retry from that marker (inclusive).

        Args:
            session: Active database session.
            sender_id: Conversation ID.
            min_event_id: Inclusive lower bound on event ``id``.

        Returns:
            Reconstructed :class:`~rasa.shared.core.trackers.DialogueStateTracker`,
            or ``None`` when build fails.
        """
        rows = self._event_query_from_id(session, sender_id, min_event_id).all()
        if not rows:
            return None

        reconstructed_tracker = self._try_build_tracker_from_rows(
            session, sender_id, rows
        )

        if reconstructed_tracker is not None:
            return reconstructed_tracker

        marker_id = self._last_stack_marker_event_id_from_rows(rows)
        if marker_id is None:
            structlogger.debug(
                "sql_tracker_store._build_tracker_from_event_id.no_stack_marker_found",
                sender_id=sender_id,
                min_event_id=min_event_id,
                row_count=len(rows),
            )
            return None
        marker_rows = self._event_query_from_id(session, sender_id, marker_id).all()
        if not marker_rows:
            structlogger.debug(
                "sql_tracker_store._build_tracker_from_event_id.no_marker_rows",
                sender_id=sender_id,
                min_event_id=min_event_id,
                marker_id=marker_id,
            )
        ...
        return self._try_build_tracker_from_rows(session, sender_id, marker_rows)

    def _replay_safe_tracker_from_db(
        self, session: "Session", sender_id: str
    ) -> Optional[DialogueStateTracker]:
        """Fetch the minimum replay-safe event prefix for ``sender_id``.

        Uses progressive session-boundary widening to minimise DB I/O while
        preserving the v3.16 replay-safe session-slicing correctness.

        The boundary type is determined by the domain session configuration:

        * ``start_session_after_expiry=True`` — boundaries are
          ``ActionExecuted(action_session_start)`` event IDs.
        * ``start_session_after_expiry=False`` — boundaries are
          ``ConversationInactive`` event IDs.

        **Algorithm:**

        1. Query session-boundary IDs for ``sender_id``, newest first.
        2. Try at most :data:`_MAX_SESSION_BOUNDARY_ATTEMPTS` boundaries
           (newest → older).  For each boundary, call
           :meth:`_build_tracker_from_event_id`; if build raises a patch error,
           that helper scans stack events in the fetched prefix for the last
           marker whose patch applies to an empty stack, then retries from that
           event.  Return on the first success.
        3. **Full history** — last resort when all attempted boundaries fail.

        Returns ``None`` when no events are stored for ``sender_id``.
        ...
        Returns:
            Replay-safe :class:`~rasa.shared.core.trackers.DialogueStateTracker`
            slice, or ``None`` when no events are stored.
        """
        start_session_after_expiry = (
            self.domain.session_config.start_session_after_expiry
        )

        # --- Progressive session-boundary widening ---
        boundary_ids = self._session_boundary_event_ids(session, sender_id)
        for i, boundary_id in enumerate(boundary_ids):
            min_event_id = self._min_fetch_event_id_for_session_boundary(boundary_id)
            tracker = self._build_tracker_from_event_id(
                session, sender_id, min_event_id
            )
            if tracker is not None:
                if i > 0:
                    structlogger.debug(
                        "sql_tracker_store._replay_safe_tracker_from_db"
                        ".widened_to_session_boundary",
                        sender_id=sender_id,
        ...
        # --- Full history (last resort) ---
        structlogger.debug(
            "sql_tracker_store._replay_safe_tracker_from_db.widening_to_full_history",
            sender_id=sender_id,
        )
        all_rows = self._event_query_from_id(session, sender_id, None).all()
        if not all_rows:
            return None

        stored_tracker = self._build_stored_tracker(session, sender_id, all_rows)
    ...
    def _additional_events(
        ...
        The offset into ``tracker.events`` is ``len(stored_sliced.events)`` where
        ``stored_sliced`` is the replay-safe slice returned by
        :meth:`_replay_safe_tracker_from_db` — the same slice that
        :meth:`retrieve` would have returned.  This guarantees that the offset
        is consistent with what the caller processed before adding new events.

        The ``domain.is_empty()`` early-exit path uses a raw SQL row count
        rather than replay-safe slicing, because domain-less operation has no
        slot definitions and cannot reconstruct a tracker meaningfully.
        """
        ...
        # Delegate to the shared replay-safe tracker lookup used by retrieve().
        stored_sliced = self._replay_safe_tracker_from_db(session, tracker.sender_id)
        if stored_sliced is None:
            # No events stored at all — this is the first save.
            return itertools.islice(tracker.events, 0, len(tracker.events))

        return itertools.islice(
            tracker.events, len(stored_sliced.events), len(tracker.events)
        )
