...
CONCURRENT_KAFKA_BROKER_TYPE = "concurrent_kafka"
...
class KafkaEventBroker(EventBroker):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # Guards lazy producer init and reconnect so concurrent publish paths don't
        # create or replace the producer twice.
...
class ConcurrentKafkaEventBroker(KafkaEventBroker):
    """A non-blocking Kafka event broker that offloads publishing to a thread pool.

    Intended to be subclassed for customer-specific filtering: override ``publish()``,
    apply any drop/transform logic synchronously, then call ``super().publish(...)`` so
    the parent handles executor offload and concurrency.

    Accepts all configuration options of ``KafkaEventBroker`` plus:
        executor_max_workers: Background publish threads (default: 1). Values greater
            than 1 are supported; the confluent_kafka ``Producer`` is thread-safe for
            ``produce()``, but concurrent retries can reorder events for the same
            sender relative to their timestamps.

    Note: ``publish()`` defaults to ``retries=3`` (not the parent's 60) so that a
    background thread does not sleep for up to 5 minutes on repeated failures.
    Use type: ``concurrent_kafka`` in endpoint config to select this broker.
    """

    def __init__(self, *args: Any, **kwargs: Any) -> None:
        max_workers = kwargs.pop("executor_max_workers", 1)
        super().__init__(*args, **kwargs)
        self._executor = ThreadPoolExecutor(
            max_workers=max_workers,
            thread_name_prefix="kafka-publish-",
        )
    ...
    def publish(
        ...
        """Fire-and-forget publish that offloads to a thread pool."""
        try:
            loop = asyncio.get_running_loop()
            # run_in_executor returns an asyncio.Future wrapping the thread work.
            # We intentionally drop it: the Future is not awaited because this is a
            # fire-and-forget call. The executor still tracks and runs the submitted
            # thread, and close() drains it via executor.shutdown(wait=True).
            ...
            # No running loop, or loop/executor is shutting down.
            try:
                ...
                logger.debug("Kafka publish executor is shut down; dropping event.")
    ...
    def _safe_publish(
        ...
        """Wrapper that catches and logs all exceptions from background publish."""
        try:
            super().publish(event, retries, retry_delay_in_seconds)
        except Exception as exc:
            structlogger.error(
                "rasa.core.brokers.kafka.ConcurrentKafkaEventBroker.publish",
                event_info="Background Kafka publish failed.",
                url=self.url,
                error=str(exc),
            )

    async def close(self) -> None:
        """Drain in-flight publishes then close the producer."""
        if not self._executor_shutdown:
            try:
                await asyncio.wait_for(
                    asyncio.to_thread(self._executor.shutdown, True),
                    timeout=30,
                )
            except asyncio.TimeoutError:
                structlogger.warning(
                    "rasa.core.brokers.kafka.ConcurrentKafkaEventBroker.close",
                    event_info="Kafka executor did not drain within 30s; proceeding with close.",  # noqa: E501
                )
                # Cancel queued-but-not-yet-running tasks. Already-running threads
                # are not interruptible and may still touch the producer until done.
                try:
                    self._executor.shutdown(wait=False, cancel_futures=True)
                except RuntimeError:
                    structlogger.warning(
                        "rasa.core.brokers.kafka.ConcurrentKafkaEventBroker.close",
                        event_info="Forced executor shutdown failed; some tasks may still be running.",  # noqa: E501
                    )
