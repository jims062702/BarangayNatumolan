...
class RasaLLMClient(_BaseLiteLLMClient):
    ...
    def _completion_fn_args(self) -> Dict[str, Any]:
        ...
        update_dict: Dict[str, Any] = {LITE_LLM_API_BASE_FIELD: self.api_base}
        # Only fall back to the Rasa license when no api_key was explicitly configured.
        # An explicit api_key in the model config takes precedence over the license,
        # which is required when pointing api_base at a non-Rasa endpoint.
        if LITE_LLM_API_KEY_FIELD not in self._extra_parameters:
            license_value, _ = retrieve_license_from_env()
            update_dict[LITE_LLM_API_KEY_FIELD] = license_value
        fn_args.update(update_dict)
        return fn_args
