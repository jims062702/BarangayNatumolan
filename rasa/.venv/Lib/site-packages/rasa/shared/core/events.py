...
def deserialise_events(serialized_events: List[Dict[Text, Any]]) -> List["Event"]:
    ...
    Malformed or incomplete event dicts are skipped with a warning so the rest
    of the tracker can still be loaded.
    ...
    for e in serialized_events:
        if "event" not in e:
            structlogger.warning("event.deserialization.missing_event_key")
            continue
        try:
...
def split_events(
    ...
    Args:
        ...
        additional_splitting_conditions: Additional event attributes to split on.
            Scalar values must match exactly. Dict values are matched as subsets:
            every key in the condition must be present and equal on the event
            attribute, but the event may contain additional keys (for example
            runtime-injected metadata not included in the search conditions).
    ...
    def condition_value_matches(actual_value: Any, expected_value: Any) -> bool:
        # dict-valued conditions (e.g. event metadata) match as a subset: every
        # key/value in the expected dict must be present and equal in the
        # actual dict, but the actual dict may contain additional keys (such
        # as runtime-injected metadata the caller doesn't know about).
        if isinstance(expected_value, dict) and isinstance(actual_value, dict):
            return all(
                key in actual_value and actual_value[key] == value
    ...
    def event_fulfills_splitting_condition(evt: "Event") -> bool:
        ...
        return all(
            condition_value_matches(getattr(evt, k, None), v)
...
class TTSFinished(SkipEventInMDStoryMixin):
    """TTS synthesis completed for a bot utterance.

    The metadata contains TTS timing information such as time to first byte
    ...
    def __repr__(self) -> Text:
        ...
        return f"TTSFinished(metadata: {self.metadata})"
...
class AgentProcessing(SkipEventInMDStoryMixin):
    """Mark that an agent is still processing."""

    type_name = "agent_processing"
    ...
    def __repr__(self) -> str:
        """Returns event as string for debugging."""
        return f"AgentProcessing(agent: {self.agent_id}, flow: {self.flow_id})"
    ...
    def __eq__(self, other: Any) -> bool:
        """Compares object with other object."""
        if not isinstance(other, AgentProcessing):
            return NotImplemented
        return (self.agent_id, self.flow_id) == (other.agent_id, other.flow_id)
    ...
    @classmethod
    def _from_parameters(cls, parameters: Dict[str, Any]) -> AgentProcessing:
        if "agent_id" not in parameters:
            raise ValueError(
                "Failed to parse agent_resumed event: agent_id is required"
            )
...
class BotTurnEnded(SkipEventInMDStoryMixin):
    """The bot's turn has ended.

    The metadata contains latency information for the bot's turn, such as
    command processor time and prediction loop time.
    """
    ...
    def __repr__(self) -> Text:
        """Returns event as string for debugging."""
        return f"BotTurnEnded(metadata: {self.metadata})"
