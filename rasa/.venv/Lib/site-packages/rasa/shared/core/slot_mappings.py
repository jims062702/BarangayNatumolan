...
def slot_mappings_allow_llm_fill(
    mappings: Optional[Sequence[Union[Dict[str, Any], SlotMapping]]] = None,
) -> bool:
    """Return whether the LLM may fill the slot based on mappings."""
    if not mappings:
        return True

    for mapping in mappings:
        if isinstance(mapping, SlotMapping):
            if mapping.type == SlotMappingType.FROM_LLM:
                return True
            continue

        mapping_type = mapping.get(KEY_MAPPING_TYPE, SlotMappingType.FROM_LLM.value)
        if mapping_type == "custom":
            mapping_type = SlotMappingType.CONTROLLED.value
        if mapping_type == SlotMappingType.FROM_LLM.value:
            return True
