...
class DialogueStateTracker:
    ...
    def __init__(
        ...
        # Ephemeral per-turn guard: not persisted across tracker loads.
        self._session_start_executed_this_turn = False
    ...
    def has_coexistence_routing_slot(self) -> bool:
        ...
        if isinstance(self.slots, AnySlotDict):
            # AnySlotDict means the tracker was created without domain slots,
            # so the coexistence routing slot was never defined.
            # AnySlotDict.__contains__ always returns True, so we can't use `in`.
            return False
        # if slots are defined in the domain, self.slots is a dict, so we can use `in`
    ...
    def latest_sub_session_events(self) -> List[Event]:
        """Return events in the latest ``action_session_start`` sub-session."""
        if not self.events:
            return []
        ...
        return list(sub_sessions[-1])

    def mark_session_start_executed_this_turn(self) -> None:
        """Record that ``action_session_start`` succeeded in the current turn."""
        self._session_start_executed_this_turn = True

    def is_session_time_expired(self, session_expiration_time: float) -> bool:
        """Return whether the session has expired by wall-clock time.

        Uses the timestamp of the last ``UserUttered`` event and the configured
        session expiration window. Legacy trackers without a ``ConversationInactive``
        event are treated as expired when this returns ``True``.
        """
        if session_expiration_time <= 0:
            return False

        user_uttered_event = self.get_last_event_for(UserUttered)
        if not user_uttered_event or not isinstance(user_uttered_event, UserUttered):
            return False

        expiration_seconds = session_expiration_time * 60
        return time.time() > user_uttered_event.timestamp + expiration_seconds

    def has_session_started_for_current_message(
        self,
        session_expiration_time: Optional[float] = None,
    ) -> bool:
        ...
        Returns ``False`` if the session is time-expired (allowing restart for
        legacy trackers without ``ConversationInactive``), if a
        ``ConversationInactive`` or ``Restarted`` event is encountered first, or
        if the tracker is inactive or terminated.
        """
        if self._session_start_executed_this_turn:
            return True

        if self.inactive or self.terminated:
            return False

        if session_expiration_time is not None and self.is_session_time_expired(
        ...
        latest_session = self.latest_sub_session_events()
    ...
    def _prepare_event_metadata(self, event: Event, is_replay: bool) -> None:
        ...
        # inject default_language into metadata if it exists
        try:
            metadata_default_language = self.default_language.code
            if (
                metadata_default_language
                and METADATA_DEFAULT_LANGUAGE not in event.metadata
            ):
                event.metadata = {
                    **event.metadata,
                    METADATA_DEFAULT_LANGUAGE: metadata_default_language,
                }
    ...
    def supported_languages(self) -> List[Language]:
        ...
        """
        if isinstance(self.slots, AnySlotDict) or LANGUAGE_SLOT not in self.slots:
            raise RasaException(
