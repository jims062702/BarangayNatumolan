...
def _hash_slot_rejections(
    rejections: List[SlotRejection],
) -> tuple[tuple[str, str], ...]:
    return tuple((rejection.if_, rejection.utter) for rejection in rejections)


def _hash_silence_timeout(
    silence_timeout: Optional[SilenceTimeout],
) -> Optional[tuple[Any, ...]]:
    if silence_timeout is None:
        return None
    if isinstance(silence_timeout, SingleSilenceTimeout):
        return ("single", silence_timeout.silence_timeout)
    if isinstance(silence_timeout, PerChannelSilenceTimeout):
        return (
            "per_channel",
            frozenset(silence_timeout.silence_timeouts.items()),
        )
    raise TypeError(f"Unsupported silence timeout type: {type(silence_timeout)}")


def _hash_dtmf(dtmf: Optional[DTMFConfig]) -> Optional[tuple[Any, ...]]:
    if dtmf is None:
        return None
    return (dtmf.length, dtmf.finish_on_key, dtmf.allow_audio_input)
    ...
    def __hash__(self) -> int:
        ...
        return hash(
            (
                ...
                self.utter,
                self.collect_action,
                _hash_slot_rejections(self.rejections),
                self.ask_before_filling,
                self.reset_after_flow_ends,
                self.force_slot_filling,
                _hash_silence_timeout(self.silence_timeout),
                _hash_dtmf(self.dtmf),
                self.idx,
                self.description,
                self.flow_id,
            )
