...
def validate_secrets_in_params(
    params: Dict[str, Any],
    context_name: str = "configuration",
    additional_secret_keys: Optional[Iterable[str]] = None,
) -> None:
    ...
    Args:
        ...
        additional_secret_keys: Extra parameter names to treat as secrets.
            List values are validated item-by-item (e.g. ``valid_keys``).
    """
    secret_keys = set(AUTH_SECRETS)
    if additional_secret_keys is not None:
        secret_keys.update(additional_secret_keys)

    for key, value in params.items():
        if key in secret_keys:
            if isinstance(value, list):
                for item in value:
                    _validate_secret_value(item, key, context_name)
            else:
