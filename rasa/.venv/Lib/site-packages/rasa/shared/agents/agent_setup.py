...
async def _close_litellm_sessions() -> None:
    """Close litellm's shared httpx sessions before the event loop shuts down.

    On Windows (ProactorEventLoop) an unclosed AsyncClient is finalised by GC
    after asyncio.run() closes the loop, producing a spurious "Fatal error on
    SSL transport" traceback (ENG-2766). Resetting to None restores litellm's
    lazy-init behaviour for subsequent calls.
    """
    try:
        if isinstance(litellm.aclient_session, httpx.AsyncClient):
    ...
    try:
        if isinstance(litellm.client_session, httpx.Client):
...
class AgentsConnectionCleanup:
    ...
    async def __aexit__(
        ...
        # Close litellm's shared httpx sessions while the event loop is still
        # alive. On Windows (ProactorEventLoop) an unclosed AsyncClient is
        # finalised by GC after asyncio.run() closes the loop, triggering a
        # spurious "Fatal error on SSL transport" traceback (ENG-2766).
        # Must run after agent disconnect: sub-agent teardown may use litellm.
        await _close_litellm_sessions()
