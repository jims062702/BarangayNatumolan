...
def _add_env_var_constructor() -> None:
    """Register a process-wide fallback constructor for the '!env_var' tag.

    _add_env_var_resolver() registers "!env_var" on ruamel's module-level,
    process-global implicit_resolvers list, so ANY ruamel.yaml.YAML() built
    anywhere in this process after this module is imported will tag unquoted
    ${VAR} scalars as !env_var - whether or not that parser is ours (e.g.
    rasa_sdk's tracing-config loader, which builds a bare YAML(typ=...) and
    has no matching constructor). Without a fallback, that raises
    ConstructorError. This closes that gap the same way the resolver itself
    is global.

    The fallback deliberately does NOT expand the value. A foreign parser
    has no notion of Rasa's SENSITIVE_DATA / DEFERRED_RESOLUTION_KEYS
    protections, so an always-expanding fallback would risk exposing
    secrets to any bare-parser consumer in the process (including Rasa's
    own e2e YAML utilities, which also build bare parsers). Rasa's own
    parsers (create_yaml_parser) always override this default on their own
    per-parser Constructor subclass, so this has no effect on rasa's own
    env-var-expansion semantics.
    """
    for constructor_cls in (
        yaml.Constructor,
        yaml.SafeConstructor,
        yaml.RoundTripConstructor,
        yaml.Loader,
        yaml.SafeLoader,
    ):
        constructor_cls.add_constructor("!env_var", _no_expand_env_var_constructor)


def _no_expand_env_var_constructor(loader: BaseConstructor, node: ScalarNode) -> str:
    """Return the raw '!env_var' scalar without expanding it."""
...
def environment_variables_replaced(
    ...
    try:
        ...
        yaml_parser.constructor.yaml_constructors["!env_var"] = (
            _no_expand_env_var_constructor
        )
...
def create_yaml_parser(
    ...
    # Give every rasa-owned parser an explicit, non-expanding default for
    # "!env_var", independent of the process-global fallback registered by
    # _add_env_var_constructor(). Without this, a parser created here would
    # inherit that global (always-expanding) default via the dict copy
    # above, and expand_env_vars=False callers (e.g. Studio push/upload,
    # which deliberately avoid expansion to keep secrets out of pushed
    # content) would silently start expanding env vars instead of leaving
    # them literal. environment_variables_replaced() still overrides this
    # to the expanding constructor only when expand_env_vars=True.
    yaml_parser.constructor.add_constructor("!env_var", _no_expand_env_var_constructor)
...
def _read_yaml_file_impl(
    filename: Union[str, Path],
    reader_type: Union[str, Tuple[str, ...]] = "safe",
    expand_env_vars: bool = True,
) -> Union[List[Any], Dict[str, Any]]:
    """Shared parse logic for read_yaml_file. Not part of the public API."""
    try:
        fixed_reader_type = (
...
def _read_yaml_file_cached(
    filename: Union[str, Path],
    reader_type: Union[str, Tuple[str, ...]] = "safe",
    expand_env_vars: bool = True,
) -> Union[List[Any], Dict[str, Any]]:
    """LRU-cached version of _read_yaml_file_impl. Not part of the public API.

    Test fixtures that need to clear or inspect the cache should use
    _read_yaml_file_cached.cache_clear() / .cache_info() directly.
    """
    return _read_yaml_file_impl(filename, reader_type, expand_env_vars)
...
def read_yaml_file(
    ...
    Args:
        ...
        reader_type: Reader type to use. By default "safe" will be used.
        expand_env_vars: Whether to expand environment variables in the file.
        skip_cache: When True, always read from disk and bypass the LRU cache.
            Use this in long-running processes where the file may be edited
            between calls (e.g. the MCP server). Defaults to False.
    ...
    if skip_cache:
        return _read_yaml_file_impl(filename, reader_type, expand_env_vars)
    return _read_yaml_file_cached(filename, reader_type, expand_env_vars)
