...
from rasa.shared.providers._litellm_config import configure_litellm_for_rasa
...
async def acompletion_with_streaming(
    ...
    **kwargs: Any,
    ...
    Both *output_channel* and *recipient_id* must be supplied for streaming to
    work. If either is missing or falsy, :class:`ValueError` is raised immediately.
    The parameters are typed as optional only so callers can pass explicit ``None``
    in conditional code paths; omitting them is not supported.
    ...
    Args:
        ...
        output_channel: Output channel to send streaming chunks to. Must be
            provided together with *recipient_id*; ``None`` raises
            :class:`ValueError`.
        recipient_id: Recipient ID for the output channel. Must be provided
            together with *output_channel*; ``None`` or an empty string raises
            :class:`ValueError`.
    ...
    Raises:
        ValueError: If *output_channel* or *recipient_id* is missing or falsy.
    """
    accumulated_text = ""
    ...
    # Guard against callers that bypass static typing (e.g. explicit ``None``).
    ...
    # Send end-of-stream signal once at least one content chunk was received.
    if llm_response_metadata.get("id"):
    ...
    if first_token_time is not None:
        response.llm_time_to_first_token_ms = (
...
def _combine_model_groups_configs_with_default_config(
    ...
    """Merges each model configuration within a model group with defaults.

    Processes model group configurations by applying the merging process to
...
def get_provider_from_config(config: dict) -> Optional[str]:
    """Returns the provider from an LLM or embeddings configuration.

    Returns None if no provider can be determined.
    """
    if not config:
        return None
...
def llm_router_factory(
    ...
    Args:
        ...
        **kwargs: Reserved for API compatibility (unused).
...
def get_model_and_provider(
    ...
    config = llm_client.config or {}
    model = config.get("model") or config.get("model_name") or config.get("deployment")
    provider = config.get("provider") or config.get("custom_llm_provider")
    return (
...
def supports_json_response_format(llm_client: LLMClient) -> bool:
    """Whether the LLM behind ``llm_client`` accepts JSON-object mode.

    Uses ``litellm.get_supported_openai_params`` (same approach Rasa uses
    elsewhere for capability detection) so we only opt into JSON mode for
    models that actually support it. Falls back to ``False`` on any
    introspection error — better to skip the parameter than break a
    provider that would have otherwise worked without it.
    """
    model, provider = get_model_and_provider(llm_client)
    if not model:
        return False
    ...
    try:
        ...
        # Defensive capability probe — never let an introspection failure
        # break the caller; just report the model as not supporting JSON mode.
        structlogger.debug(
            "utils.llm.supports_json_response_format.probe_failed",
            model=model,
            provider=provider,
            error=repr(exc),
        )
        return False
    return isinstance(supported, list) and "response_format" in supported


def supports_response_schema(llm_client: LLMClient) -> bool:
    """Whether the LLM can accept a Pydantic response schema."""
    model, provider = get_model_and_provider(llm_client)
    ...
    try:
        ...
        # Defensive capability probe — never let an introspection failure
        # break the caller; just report the model as not supporting schemas.
        structlogger.debug(
            "utils.llm.supports_response_schema.probe_failed",
            model=model,
            provider=provider,
            error=repr(exc),
        )
