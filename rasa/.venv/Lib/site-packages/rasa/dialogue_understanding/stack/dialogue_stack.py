...
class DialogueStack:
    ...
    def find_agent_stack_frame_by_task_id(
        self, task_id: str
    ) -> Optional[AgentStackFrame]:
        for frame in self.frames:
            if isinstance(frame, AgentStackFrame) and frame.agent_task_id == task_id:
                return frame

        return None

    def update_agent_stack_frame(
        self,
        agent_frame: AgentStackFrame,
    ) -> None:
        """Updates the agent stack frame on the given agent stack frame.

        Frames are matched by frame_id.
        """
        for index, frame in enumerate(self.frames):
            if frame.frame_id == agent_frame.frame_id:
                self.frames[index] = agent_frame
                return
