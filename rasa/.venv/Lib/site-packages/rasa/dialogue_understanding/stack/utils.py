...
def get_collect_steps_for_active_flow(
    dialogue_stack: DialogueStack, all_flows: FlowsList
) -> Tuple[Set[CollectInformationFlowStep], Optional[str]]:
    """Get all collect steps that are currently on the stack."""
    ...
    collect_steps = {
        step for active_flow in active_flows for step in active_flow.get_collect_steps()
    }
    flow_id = active_primary_frame.flow_id if active_primary_frame else None
    return collect_steps, flow_id
...
def get_collect_steps_excluding_ask_before_filling_for_active_flow(
    ...
    """
    collect_steps, _ = get_collect_steps_for_active_flow(dialogue_stack, all_flows)

    return {step.collect for step in collect_steps if not step.ask_before_filling}
