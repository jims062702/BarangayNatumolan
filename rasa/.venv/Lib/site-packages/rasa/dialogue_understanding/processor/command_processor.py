...
from rasa.shared.core.slot_mappings import SlotMapping, slot_mappings_allow_llm_fill
...
def get_commands_from_tracker(tracker: DialogueStateTracker) -> List[Command]:
    ...
    if tracker.latest_message:
        ...
        return Command.commands_from_json(dumped_commands)
    else:
        return []
...
def get_current_collect_step(
    ...
    if step is None:
        # The underlying flow's steps have changed and the frame now points
        # at a step that no longer exists (e.g. after a flow rename across
        # an upgrade). _purge_stale_user_flow_frames is the primary cleanup;
        # this guard prevents a crash when it didn't run or didn't detect
        # the change (e.g. missing FLOW_HASHES_SLOT entry).
        structlogger.warning(
            "command_processor.get_current_collect_step.stale_frame",
            flow_id=frame_that_triggered_collect_infos.flow_id,
            step_id=frame_that_triggered_collect_infos.step_id,
        )
        return None
...
def should_slot_be_set(
    ...
    if command.extractor in {
        SetSlotExtractor.COMMAND_PAYLOAD_READER.value,
        SetSlotExtractor.A2A_CLIENT.value,
    }:
        # Deterministic external sources (button payloads, A2A orchestrator metadata).
        return True

    if commands_so_far is None:
        # First command in this cleanup pass for the slot.
        commands_so_far = []
    ...
    if not slot.mappings:
        # Unconfigured slot: default to from_llm for checks below.
    ...
    slot_allows_llm_fill = slot_mappings_allow_llm_fill(slot_mappings)
    ...
    if set_slot_commands_so_far and command.extractor == SetSlotExtractor.LLM.value:
        ...
        return not command_has_nlu_extractor and slot_allows_llm_fill

    if (
        slot_has_nlu_mapping
        and command.extractor == SetSlotExtractor.LLM.value
        and not slot_allows_llm_fill
    ):
        # NLU-mapped slot without from_llm: reject LLM extractor.
        return False

    if (
        slot_allows_llm_fill
        and command.extractor == SetSlotExtractor.NLU.value
        and not slot_has_nlu_mapping
    ):
        # from_llm-only slot: reject NLU extractor.
        return False

    if slot_has_controlled_mapping and not (
        slot_has_nlu_mapping or slot_allows_llm_fill
    ):
        # Controlled-only slot: reject without NLU or from_llm mapping.
        return False
