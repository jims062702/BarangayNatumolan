...
class ContinueInterruptedPatternFlowStackFrame(PatternFlowStackFrame):
    ...
    def from_dict(data: Dict[str, Any]) -> ContinueInterruptedPatternFlowStackFrame:
        ...
        """
        interrupted_flow_names = data.get("interrupted_flow_names", [])
        require_keys(
            data, "frame_id", "step_id", context="Continue interrupted pattern frame"
        )
        return ContinueInterruptedPatternFlowStackFrame(
            frame_id=data["frame_id"],
            step_id=data["step_id"],
            interrupted_flow_names=interrupted_flow_names,
            interrupted_flow_ids=data.get("interrupted_flow_ids", []),
            interrupted_flow_options=data.get("interrupted_flow_options", ""),
            multiple_flows_interrupted=len(interrupted_flow_names) > 1,
        )
