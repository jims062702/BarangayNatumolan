...
class MultiStepLLMCommandGenerator(LLMBasedCommandGenerator):
    ...
    def _resolve_prompt_template(
        ...
        """Determine and retrieve a prompt template for the given step key.

        If the prompt associated with the key is missing in both the
        `prompt_templates` and the `config`, defaults to a predefined template.
        Each key is uniquely associated with a distinct step of the command
        generation process.
    ...
    def _load_prompt_templates(
        ...
        """Load prompt templates from model storage, falling back to defaults."""
        prompts = {}
    ...
    @observe(
        name="command_generator.multi_step.active_flow",
        as_type="generation",
        capture_input=False,
    ...
    async def _predict_commands_for_active_flow(
        ...
        update_current_span_metadata(
            metadata={
                "prompt_type": "command_generation_active_flow",
                "command_generator": self.__class__.__name__,
    ...
    @observe(
        name="command_generator.multi_step.handle_flows",
        as_type="generation",
        capture_input=False,
        capture_output=False,
    )
    async def _predict_commands_for_handling_flows(
        ...
        update_current_span_metadata(
            metadata={
                "prompt_type": "command_generation_handle_flows",
                "command_generator": self.__class__.__name__,
                "sender_id": tracker.sender_id,
            }
        )
    ...
    def _filter_redundant_start_flow_commands(
        ...
        """Filter out StartFlowCommands for flows already active in the tracker."""
    ...
    @observe(
        name="command_generator.multi_step.new_flow",
        as_type="generation",
        capture_input=False,
        capture_output=False,
    )
    async def _predict_commands_for_newly_started_flow(
        ...
        update_current_span_metadata(
            metadata={
                "prompt_type": "command_generation_new_flow",
                "command_generator": self.__class__.__name__,
                "sender_id": tracker.sender_id,
                "flow_id": newly_started_flow.id,
            }
        )
    ...
    def _clean_up_commands(commands: List[Command]) -> List[Command]:
        """Remove CannotHandleCommand from the list if other commands are present."""
