"""Implements the `FlowRetrieval` class for semantic flow search and filtering.

The module is primarily centered around the `FlowRetrieval` class which handles
...
from rasa.core.information_retrieval.faiss import (
    DISTANCE_METRIC_INNER_PRODUCT,
    FAISS_Store,
    LegacyFaissModelError,
)
...
from rasa.shared.utils.langfuse import get_langfuse_session_id
...
from rasa.utils.ml_utils import load_faiss_vector_store
...
class FlowRetrieval(EmbeddingsHealthCheckMixin):
    ...
    def __init__(
        ...
        self.vector_store: Optional[FAISS_Store] = None
    ...
    ) -> Optional[FAISS_Store]:
        """Loads a FAISS vector store from a specified local path.

        Legacy LangChain-pickled models are detected and rejected with a
        retrain error. The error propagates so startup fails fast rather
        than silently degrading.
        """
        embeddings = cls._create_embedder(config)
        try:
            with model_storage.read_from(resource) as model_path:
                return load_faiss_vector_store(model_path, embeddings)
        except LegacyFaissModelError:
            raise
    ...
    @classmethod
    def _create_embedder(cls, config: Dict[Text, Any]) -> EmbeddingClient:
        """Creates an embedder.
    ...
    def _persist_vector_store(self) -> None:
        ...
        if self.vector_store is not None:
            with self._model_storage.write_to(self._resource) as model_path:
                self.vector_store.save(model_path)
    ...
    def get_llm_tracing_metadata(self, tracker: DialogueStateTracker) -> Dict[str, Any]:
        return {
            LANGFUSE_METADATA_SESSION_ID: get_langfuse_session_id(tracker),
    ...
    def populate(self, flows: FlowsList, domain: Domain) -> None:
        """Populate the vector store with embeddings generated from flow and slot docs.
        ...
        try:
            self.vector_store = FAISS_Store.from_documents(
                documents=documents,
    ...
    def _generate_flow_documents(
        ...
        """Convert each flow into an embeddable document.
        ...
        for flow in flows:
            ...
            flow_docs.append(
                Document(
                    text=Template(self.flow_document_template)
                    .render(flow_info)
                    .strip(),
    ...
    async def filter_flows(
        ...
        """Filter flows by always-included, previously-started, and semantic similarity.
    ...
    async def find_most_similar_flows(
        ...
        """Return the top 'k' flows most similar to the current conversation.

        Args:
            tracker: the tracker
            message: the user message
            flows: available flows to search within
        ...
        for doc, score in documents_with_scores:
            flow_id = (doc.metadata or {}).get("flow_id")
            if flow_id is None:
                structlogger.warning("flow_retrieval.missing_flow_id_metadata")
                continue
            if (flow := flows.flow_by_id(flow_id)) is not None:
                most_similar_flows_with_scores.append((flow, score))
    ...
    def _prepare_query(self, tracker: DialogueStateTracker, message: Message) -> str:
        """Prepare the vector store query from the last N conversation turns.
    ...
    async def _query_vector_store(
        ...
        """Compare the query against all flows and return the top k most similar.

        Args:
            query: query
            tracker: the tracker for LLM tracing metadata
        ...
        try:
            ...
            structlogger.debug(
                ...
                results=[
                    {
                        "flow_id": (document.metadata or {}).get("flow_id"),
                        "score": score,
