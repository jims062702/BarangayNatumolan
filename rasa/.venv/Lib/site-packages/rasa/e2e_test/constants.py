...
EVAL_CONFTEST_DIR = "eval"
# Entries at the assistant project root — file or directory (checked with Path.exists).
ASSISTANT_PROJECT_ROOT_ENTRIES = ("domain", "config.yml", "domain.yml", "config")


def is_assistant_project_root(path: Path) -> bool:
    """Return whether *path* is the assistant project root directory."""
    return any((path / entry).exists() for entry in ASSISTANT_PROJECT_ROOT_ENTRIES)


def is_eval_conftest_path(path: Path) -> bool:
    """Return whether *path* is `<project>/eval/conftest.yml` (not E2E)."""
    parent = path.parent
    return (
        path.name in CONFTEST_FILENAMES
        and parent.name == EVAL_CONFTEST_DIR
        and is_assistant_project_root(parent.parent)
    )
