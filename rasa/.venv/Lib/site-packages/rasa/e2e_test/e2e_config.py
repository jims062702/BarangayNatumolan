...
def get_conftest_path(test_case_path: Optional[Path]) -> Optional[Path]:
    ...
    while True:
        ...
        if is_assistant_project_root(test_case_path):
            return None

        parent = test_case_path.parent
        if parent == test_case_path:
            # Filesystem root (or a relative path with no parent)
            return None

        test_case_path = parent


def find_conftest_path(path: Path) -> Generator[Path, None, None]:
    """Yield E2E conftest files located directly in *path* (not in subdirectories)."""
    for pattern in CONFTEST_PATTERNS:
        candidate = path / pattern
        if candidate.is_file() and not is_eval_conftest_path(candidate):
            yield candidate
