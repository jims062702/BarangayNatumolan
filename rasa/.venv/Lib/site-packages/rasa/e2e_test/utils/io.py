...
def get_conftest_files_for_directory(dir_path: str) -> List[str]:
    ...
    while current:
        for name in CONFTEST_FILENAMES:
            ...
            if os.path.isfile(p) and not is_eval_conftest_path(Path(p)):
                chain.append(p)
                break
        current = (
...
def _get_relative_output_path(
    test_case_file: Optional[str], input_folder: str
) -> Tuple[Optional[str], str]:
    """Compute the relative (subdir, basename) for a test case's source file.

    Mirrors the source folder structure under an output directory so that a
    multi-file suite can be dumped without collapsing per-file fixture scoping
    (ENG-2647).

    Args:
        test_case_file: The `TestCase.file` value (absolute or relative path).
        input_folder: Root folder of the original suite. Relative paths are
            computed against this root.

    Returns:
        Tuple of `(relative_subdir, file_name)`. `relative_subdir` is `None`
        when the file sits directly in `input_folder` or `test_case_file`
        falls outside it (in which case `file_name` is the bare basename).
    """
    if not test_case_file:
        return None, "_unsourced.yml"

    file_path = os.path.normpath(test_case_file)
    root = os.path.normpath(input_folder)

    if file_path == root or os.path.dirname(file_path) == root:
        return None, os.path.basename(file_path)

    root_with_sep = root + os.sep
    if file_path.startswith(root_with_sep):
        remainder = file_path[len(root_with_sep) :]
        subdir = os.path.dirname(remainder) or None
        return subdir, os.path.basename(remainder)

    return None, os.path.basename(file_path)


def write_test_suite_to_directory(
    test_suite: TestSuite,
    output_folder: str,
    input_folder: str,
) -> None:
    """Write a multi-source TestSuite to a directory, mirroring source layout.

    Groups test cases by `TestCase.file`, builds a single-source `TestSuite`
    per group (with only the fixtures and metadata that group references),
    and writes one schema-conformant YAML per source file under
    `output_folder`. The relative directory structure of `input_folder` is
    preserved so reloading the output via `read_test_cases` reproduces the
    original per-file fixture scoping (ENG-2647).

    Args:
        test_suite: The suite to serialize. May span multiple source files.
        output_folder: Directory to write into. Created if missing.
        input_folder: Root of the original test sources, used to compute
            relative paths for the output tree.
    """
    Path(output_folder).mkdir(parents=True, exist_ok=True)

    test_cases_by_file: Dict[Optional[str], List[Any]] = {}
    for test_case in test_suite.test_cases:
        test_cases_by_file.setdefault(test_case.file, []).append(test_case)

    for source_file, group_test_cases in test_cases_by_file.items():
        group_keys = {(tc.name, tc.file) for tc in group_test_cases}
        group_fixtures_per_test = [
            tcf
            for tcf in test_suite.fixtures_per_test
            if (tcf.test_case_name, tcf.file) in group_keys
        ]

        # Metadata and stub_custom_actions are emitted unfiltered in every
        # per-file YAML: `extract_metadata` dedups by name on reload (see
        # `rasa/e2e_test/utils/io.py:extract_metadata`), so unreferenced
        # entries round-trip without colliding.
        group_suite = TestSuite(
            test_cases=group_test_cases,
            fixtures_per_test=group_fixtures_per_test,
            metadata=test_suite.metadata,
            stub_custom_actions=test_suite.stub_custom_actions,
        )
        data = {k: v for k, v in group_suite.as_writable().items() if v}

        subdir, file_name = _get_relative_output_path(source_file, input_folder)
        target_dir = Path(output_folder)
        if subdir:
            target_dir = target_dir / subdir
        target_dir.mkdir(parents=True, exist_ok=True)
        rasa.utils.io.write_yaml(data, target=str(target_dir / file_name))
...
def write_failed_tests_to_file(
    ...
    """Write failed tests to a directory mirroring the input folder structure.

    One YAML per source file is emitted under `output_dir` so that per-file
    fixture scoping is preserved on reload (ENG-2647).

    Args:
        test_cases: List of all test cases (unused; kept for callers that
            previously relied on the signature; the failed subset is taken
            from `results`).
        fixtures_per_test: Per-file resolved fixtures from the original suite.
        metadata: List of metadata from the original suite.
        results: List of failed test results.
        output_dir: Directory to write the failed-test tree into.
        input_folder: Root of the original test sources, used to compute
            relative output paths.
    """
    del test_cases  # Failed subset comes from `results`.

    failed_test_cases = [r.test_case for r in results]
    if not failed_test_cases:
        return

    failed_keys = {(tc.name, tc.file) for tc in failed_test_cases}
    filtered_fixtures_per_test = [
        tcf
        for tcf in fixtures_per_test
        if (tcf.test_case_name, tcf.file) in failed_keys
    ]

    failed_suite = TestSuite(
        test_cases=failed_test_cases,
        fixtures_per_test=filtered_fixtures_per_test,
        metadata=metadata,
        stub_custom_actions={},
    )
    write_test_suite_to_directory(failed_suite, output_dir, input_folder)
    rasa.shared.utils.cli.print_info(
        f"Failing tests have been saved under: {output_dir}."
    )
...
def save_test_cases_to_yaml(
    ...
    """Extracts TestCases from a list of TestResults and writes them to YAML.

    Writes one YAML per original source file under
    `<output_dir>/<status>/<relative_source_path>` so that per-file fixture
    scoping is preserved on reload (ENG-2647). A single flat `<status>.yml`
    cannot represent suites whose test cases come from different source
    files with same-named-but-different fixtures.

    Args:
        test_results: Results to dump (typically passed or failed subset).
        output_dir: Root output directory (e.g. coverage_output_path).
        status: Status label used as the subdirectory name (e.g. "passed").
        test_suite: The original suite providing `fixtures_per_test` and
    ...
    result_test_cases = [result.test_case for result in test_results]
    result_keys = {(tc.name, tc.file) for tc in result_test_cases}
    filtered_fixtures_per_test = [
        tcf
        for tcf in test_suite.fixtures_per_test
        if (tcf.test_case_name, tcf.file) in result_keys
    ]

    new_test_suite = TestSuite(
        test_cases=result_test_cases,
        fixtures_per_test=filtered_fixtures_per_test,
        metadata=test_suite.metadata,
        stub_custom_actions=test_suite.stub_custom_actions,
    )

    status_output_dir = os.path.join(output_dir, status)
    write_test_suite_to_directory(new_test_suite, status_output_dir, input_folder)

    structlogger.info(
        "rasa.e2e_test.save_e2e_test_cases",
        message=f"E2e tests with '{status}' status are written under: "
        f"'{status_output_dir}'.",
    )
    if status == STATUS_PASSED:
        structlogger.info(
            "rasa.e2e_test.save_e2e_test_cases",
            message=f"You can use the directory: '{status_output_dir}' in case you "
            f"want to create training data for fine-tuning an LLM via "
