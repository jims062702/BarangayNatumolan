...
class TestSuite:
    ...
    def as_dict(self) -> Dict[Text, Any]:
        """Returns the test suite as a dictionary, each attribute serialized."""
        return {
            KEY_FIXTURES: [
    ...
    def as_writable(self) -> Dict[Text, Any]:
        ...
        Only valid for single-source suites: all test cases must share a
        `file` (or have it unset). Multi-source suites cannot be faithfully
        represented in a single schema-conformant YAML because the writable
        format strips source-file info from both fixtures and test cases, so
        same-named fixtures from different files would silently collide on
        reload. Multi-source callers must use
        `rasa.e2e_test.utils.io.write_test_suite_to_directory` instead.

        Raises:
            RasaException: When `test_cases` span more than one source file.
        """
        distinct_files = {
            test_case.file for test_case in self.test_cases if test_case.file
        }
        if len(distinct_files) > 1:
            raise RasaException(
                "Cannot serialize a TestSuite spanning multiple source files "
                "via `as_writable()` — same-named fixtures from different "
                "files would silently collide on reload. Use "
                "`rasa.e2e_test.utils.io.write_test_suite_to_directory` to "
                "emit one YAML per source file (ENG-2647). "
                f"Files: {sorted(distinct_files)}"
            )
