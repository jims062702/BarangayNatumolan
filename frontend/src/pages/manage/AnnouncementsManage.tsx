...
export default function AnnouncementsManage() {
  ...
  const create = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (eventTime) form.append("event_time", eventTime);
      if (image) form.append("image", image);
      form.append("is_published", "1");
      await api.post("/sk/announcements", form, { headers: { "Content-Type": "multipart/form-data" } });
      setOpen(false);
      setTitle("");
      setBody("");
      setLocation("");
      ...
      setEventTime("");
      setImage(null);
      setFeedback("Posted — it now appears in the News & Announcement section.");
      load();
    } catch (err) {
  ...
  return (
    <div>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="New News / Announcement" wide>
        <form onSubmit={create} className="space-y-4">
          ...
          <FormField label="Photo (optional)" hint="Shown on the News card. JPG/PNG up to 5MB.">
            <input
              type="file"
              accept="image/*"
